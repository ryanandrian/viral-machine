...
- **⏳ Langkah 3 — MENUNGGU OWNER: masukkan kunci fal di `/integrations` → tombol uji ("valid" diharapkan; "unchecked" = probe belum pasti, lapor ke Claude).**
- Langkah 4 (setelah kunci ada): aktifkan model Kling sementara → channel uji preset 8s + niche radiant_affirmations + model video → ≥3 produksi PRIVATE (presisi 6.8–9.2s ffprobe) + biaya tercatat + 1 run 60s regresi → laporan bukti → ketok aktivasi final.
