...
1. **ADDENDUM POSISI 2026-07-16:** (a) DNA radiant `style` diharmonisasi ke anggaran kata (program durasi F4, guard hanya key style); (b) +2 model video fal **Seedance 1.0 Pro/Lite** di katalog (durasi FLEKSIBEL 2–12s = bayar pas utk 8s; nonaktif menunggu Test owner, [B18]); (c) MVT = full setelan owner via UI (8s+hailuo+radiant+Gadis) — jangan disentuh; (d) turnamen 3-model owner (kini 5 kandidat) = masih SISA.
1b. **POSISI 2026-07-15 dini hari: batch fix audit DEPLOYED `6801203` (BE 01:37 + FE 01:38, sehat 0 error) — F-2 gagal-jujur live · katalog (harga video merge + form default_params + uji video) live · fosil tersapu. UJI HAILUO PERTAMA selesai oleh owner via frontend — verdict owner: "NORAK, tidak menggambarkan wanita profesional elegan mempesona di lingkungan kantor" → DNA di-upgrade **v3 OFFICE-PROFESSIONAL** (subject power-dressed eksekutif + environment kantor prestisius + gerak-wajib; strict_prohibition utuh — daya pikat dari karisma & keanggunan, bukan keterbukaan; catatan interpretasi: kata "sexy" owner diterjemahkan ALLURING/menawan dlm batas pagar aman-iklan yang owner ketok — bila owner ingin lebih, perlu ketok ulang pagar). Berikutnya: owner Test ulang (Hailuo/Kling/Veo) dgn DNA v3 → pilih pemenang. [B7] dimandatkan (lihat backlog).** (Baris lama di bawah = sejarah.) POSISI LAMA (akhir 2026-07-14 malam): 🟢 FITUR 8s LIVE UNTUK TENANT — preset 8s + niche radiant (DNA v2 "gerak-wajib+kecantikan-sampul") + 3 MODEL VIDEO aktif (kling-2.5-turbo-pro $0.07/s · veo-3.1-fast $0.10/s 9:16-natif · hailuo-02-standard $0.045/s 768p) + vendor fal + validator + gating simpan-lalu-tuntun (deadlock telur-ayam FIXED `29eff8a` deployed 23:53). Fix DNA-niche-efektif + QC-size DEPLOYED `ee125eb` 22:43 (regresi 60s LULUS: run pemulihan MVT `published` → rem terlepas). MVT SUDAH DIPULIHKAN ke snapshot asli KECUALI llm (masih openai/gpt-4o — kembalikan ke groq/llama-3.3-70b-versatile+account NULL setelah kuota harian Groq reset). SISA: (a) turnamen 3-model DIJALANKAN OWNER via frontend (Channel MVT → 8s+model+radiant → tombol Test, private) → owner pilih pemenang → run e2e final; (b) usulan MENUNGGU KETOK: sinkron harga fal via API resmi (butuh kunci platform) · audit atribusi niche/channel semua konsumen · UX koherensi atomik preset+model; (c) anomali fal INTERMITEN terdokumentasi (403 'exhausted' saat saldo ada, 1× 22:58 — opsi: tiket support fal / terima risiko / kebijakan retry ber-ketok).** (Bila baris ini basi vs REALISASI di bawah → REALISASI menang; perbarui baris ini.)
...
- **✅ Langkah 4c — KEDUA FIX DITULIS + TERVALIDASI LOKAL → ✅ DEPLOYED `ee125eb` 2026-07-14 22:43 (lihat POSISI; frasa 'BELUM deploy' lama = snapshot saat ditulis):**
  (1) **Fix fosil s85 (DNA niche efektif):** `tenant_config` — penanda `_visual_niche_loaded` di `_load_from_supabase` + `_channel_niche` dari overlay + reload `_reload_niche_visual()` di `load()` bila niche efektif (param run > niche channel > tenant) ≠ yang termuat; semantik gagal = identik blok asal (warning+kosong, nol fallback baru). **Uji 5/5:** MVT+radiant → subject ADA · MVT+legenda → DNA wayang-Nusantara asli · MVT tanpa-param → radiant (niche channel) · RAD+universe → utuh TANPA reload (nol query tambahan jalur lama) · tenant-only → utuh.
  (2) **QC ukuran sadar-durasi:** `pipeline._pre_publish_qc` — `min_size_mb = max(0.3, env×target/60)` bila preset; tanpa preset = lama. **Uji 3/3:** video render 8s nyata 3.1MB (kelas persis yang kemarin dicap gagal) → LOLOS · file korup 0.0MB → DITOLAK (ambang 0.67MB) · basis 60s=5MB utuh.
  Pengakuan tercatat: fosil s85 = kode saya (April), kelalaian sapu multi-channel = kelalaian saya (keluarga utang REMEDIASI_NICHE yang tak saya angkat saat owner bertanya berulang) — teguran keras owner 2026-07-14.
