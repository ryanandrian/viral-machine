...
- **⏳ Langkah 4b — pemantau log worker AKTIF menunggu run 8s pertama** (producer cycle otomatis; bukti yang ditunggu: STEP0 lolos → video_prompt → AIVideo submit-poll-unduh → gerbang durasi → render → QC → ffprobe 6.8–9.2s ×3 + biaya video tercatat) → lalu 1 run 60s regresi (RAD alami slot 21:00) → laporan bukti → ketok aktivasi final + pemulihan MVT.
