      ...
      {newN && (
        <div onClick={(e) => { if (e.target === e.currentTarget) setNewN(null); }} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.55)", zIndex: 80, display: "flex", alignItems: "center", justifyContent: "center", padding: "1.5rem" }}>
          <div className="card card-pad" style={{ maxWidth: 460, width: "100%" }}>
            ...
            <label style={{ display: "flex", alignItems: "center", gap: ".5rem", fontSize: "var(--text-sm)", margin: ".4rem 0" }}><input type="checkbox" checked={newN.is_base} onChange={(e) => setNewN({ ...newN, is_base: e.target.checked })} /> is_base <span className="muted"><Bi id="(niche dasar — semua paket, termasuk Trial)" en="(base niche — all plans, incl. Trial)" /></span></label>
        ...
        {cur && (<>
          ...
          <div className="nl-dpanel">
            ...
            {dtab === 1 && <>
              ...
              <div className="nl-fld" style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}><span style={{ fontSize: "var(--text-sm)" }}>is_base <span className="muted"><Bi id="(niche dasar — semua paket, termasuk Trial)" en="(base niche — all plans, incl. Trial)" /></span></span><label className="switch"><input type="checkbox" checked={!!edit.is_base} onChange={(e) => setEdit({ ...edit, is_base: e.target.checked })} /><span className="track" /><span className="thumb" /></label></div>
