...
const CFG_META: Record<string, { label: BiTxt; group: string; unit: BiTxt; desc?: BiTxt; hint?: BiTxt; options?: string[]; optionLabels?: Record<string, BiTxt>; readonly?: boolean }> = {
  ...
  // [B34 11-Sep] Lahir dari kuota transfer Supabase JEBOL (9,55 GB vs jatah 5 GB, proyek diblokir,
  // seluruh tenant berhenti). Akarnya: pemeriksaan stok berjalan 8.640×/hari padahal stok hanya
  // berubah beberapa kali sehari. Kenop ini memindahkan kendali iramanya ke tangan owner.
  producer_stock_interval_sec:  { label: { id: "Jeda Pemeriksaan Stok Video", en: "Video Stock Check Interval" }, group: G_ENGINE, unit: U_DETIK, desc: { id: "Berapa sering mesin memeriksa apakah ada channel yang stok videonya kurang, lalu memproduksi penggantinya. Makin kecil = stok lebih cepat terisi, tapi makin boros kuota transfer data Supabase. 300 detik (5 menit) sudah 2-3x lebih cepat dari laju pemakaian video.", en: "How often the engine checks whether any channel is running low on stock and produces a replacement. Lower = stock refills faster but burns more Supabase data-transfer quota. 300 seconds (5 min) is already 2-3x faster than videos are consumed." }, hint: { id: "Tidak memengaruhi tombol \"Uji sekarang\" milik tenant — antrean uji tetap diperiksa tiap 10 detik. Nilai di bawah 30 otomatis dianggap 30 (angka 0 akan melumpuhkan server).", en: "Does not affect the tenant\u2019s \"Test now\" button \u2014 the test queue is still checked every 10 seconds. Values below 30 are treated as 30 (0 would lock up the server)." } },
