...
export default function IntegrationsPage() {
  ...
  return (
    <>
      <PageHeader helpKey="integrations" icon={Globe} title={<Bi id="Kredensial & Koneksi" en="Credentials & Connections" />} subtitle={<Bi id="Isi kunci AI, hubungkan YouTube & Telegram sekali di sini — berlaku untuk semua channel Anda." en="Set AI keys, connect YouTube & Telegram once here — applies to all your channels." />} />
      ...
      <div style={{ display: "grid", gap: "1rem", maxWidth: 720 }}>
        ...
        <div className="card card-pad">
          ...
          {ytMsg?.startsWith("error:") && <div style={{ marginBottom: "0.75rem", fontSize: "var(--text-sm)", color: "var(--danger,#ef4444)" }}>{ytMsg.slice(6) === "channel_claimed" ? <Bi id="Channel YouTube ini sudah terdaftar pada akun MesinViral lain. Satu channel hanya boleh dipakai satu akun. Bila ini memang channel Anda, hubungi dukungan." en="This YouTube channel is already registered to another MesinViral account. One channel may only be used by one account. If this really is your channel, contact support." /> : ytMsg.slice(6) === "claim_check_failed" ? <Bi id="Pemeriksaan kepemilikan channel gagal — koneksi dibatalkan demi keamanan, coba lagi sebentar." en="Channel-ownership check failed — the connection was cancelled for safety, please try again shortly." /> : ytMsg.slice(6) === "identity_failed" ? <Bi id="Gagal membaca identitas channel dari Google — koneksi dibatalkan, coba lagi." en="Could not read the channel identity from Google — connection cancelled, try again." /> : <Bi id={`Koneksi Google gagal (kode: ${ytMsg.slice(6)}) — coba lagi.`} en={`Google sign-in failed (code: ${ytMsg.slice(6)}) — try again.`} />}</div>}
