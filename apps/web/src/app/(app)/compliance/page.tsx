...
import { ShieldCheck, CheckCircle, Info, Play, Mic, Layers, Anchor, RefreshCw, AlertTriangle } from "lucide-react";
import { createClient } from "@/lib/supabase/client";
import "./compliance.css";

// D20 Compliance (Phase 9.4) — DATA NYATA dari channel_insights.compliance (RLS). Gauge + 5 dimensi
// (ComplianceScorer Phase 7: niche_distribution/hook_style_spread/voice_diversity/dup_freshness/ai_disclosure).
// Jujur saat insufficient_data (dim null = "belum cukup data" — bukan angka fabricated).
    ...
    <>
      ...
      <div className="hero-row">
        <div className="gauge-card">
          ...
          <div className="sub">{loading ? "Memuat…" : insufficient ? <Bi id="Skor penuh muncul setelah cukup video produksi terkumpul (analytics-scope harian)." en="Full score appears once enough production videos accumulate." /> : <Bi id="Channel Anda dinilai aman dari risiko YouTube AI policy." en="Your channel is assessed safe from YouTube AI policy risk." />}</div>
        </div>
        ...
        {DIM_META.map(([key, idL, enL, Icon]) => {
          ...
          return (
            <div className="card card-pad dim-card" key={key}>
              <div className="head"><span className="ic" style={{ background: "var(--brand-soft)", color: "var(--brand)" }}><Icon size={17} /></span><h3><Bi id={idL} en={enL} /></h3><span className="score" style={{ color: v == null ? "var(--text-muted)" : v >= 80 ? "var(--success)" : v >= 60 ? "var(--warning)" : "var(--error)" }}>{v != null ? `${Math.round(v)}%` : "—"}</span></div>
              <div className={`dim-note ${v == null ? "" : v >= 80 ? "ok" : "warn"}`}>
                <span style={{ flex: "none" }}>{v == null ? <Info size={14} /> : v >= 80 ? <CheckCircle size={14} /> : <AlertTriangle size={14} />}</span>
                <span>{v == null ? <Bi id="Belum cukup data produksi untuk dimensi ini." en="Not enough production data for this dimension yet." /> : v >= 80 ? <Bi id="Dalam rentang sehat." en="Within healthy range." /> : <Bi id="Di bawah target — diversity guard akan menyesuaikan." en="Below target — diversity guard will adjust." />}</span>
              </div>
            </div>
          );
        })}
