      ...
      <div className="cd-header">
        ...
        <div className="cd-h-meta">
          <h1>{name0} <span className={`badge ${TONE[eff.tone]}`} style={{ fontSize: "var(--text-xs)" }}><span className="dot" /><Bi id={eff.label_id} en={eff.label_en} /></span>
            ...
            <HelpDot locationKey="channel-detail" />
          </h1>
        ...
        <div className="card card-pad" style={{ maxWidth: 560 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
            ...
            <div style={{ borderTop: "1px solid var(--border)", paddingTop: "1rem" }}>
              ...
              {ytFlash?.kind === "error" && <div style={{ marginBottom: "0.5rem", fontSize: "var(--text-sm)", color: "var(--danger,#ef4444)" }}>{ytFlash.text === "channel_claimed" ? <Bi id="Channel YouTube ini sudah terdaftar pada akun MesinViral lain. Satu channel hanya boleh dipakai satu akun. Bila ini memang channel Anda, hubungi dukungan." en="This YouTube channel is already registered to another MesinViral account. One channel may only be used by one account. If this really is your channel, contact support." /> : ytFlash.text === "claim_check_failed" ? <Bi id="Pemeriksaan kepemilikan channel gagal — koneksi dibatalkan demi keamanan, coba lagi sebentar." en="Channel-ownership check failed — the connection was cancelled for safety, please try again shortly." /> : ytFlash.text === "identity_failed" ? <Bi id="Gagal membaca identitas channel dari Google — coba lagi." en="Could not read channel identity from Google — try again." /> : <Bi id={`Koneksi Google gagal (kode: ${ytFlash.text}) — coba lagi.`} en={`Google sign-in failed (code: ${ytFlash.text}) — try again.`} />}</div>}
