...
export default function AgentLoginPage() {
  ...
  async function doLogin() {
    ...
    if (user?.app_metadata?.role !== "agent") {
      ...
      setBusy(false);
      return setErr("Akun ini bukan akun agen. / This is not an agent account.");
    }
    // Honor ?next HANYA menuju /agent (anti open-redirect). Full reload → middleware sinkron.
  ...
  return (
    <div style={{ minHeight: "100dvh", display: "grid", placeItems: "center", padding: "1.5rem", background: "var(--bg)" }}>
      <div className="auth-card" style={{ width: "min(400px, 100%)" }}>
        ...
        <h1 style={{ fontSize: "1.25rem", marginBottom: "0.25rem" }}><Bi id="Masuk portal agen" en="Agent portal sign in" /></h1>
        <p className="lead" style={{ marginBottom: "1rem" }}><Bi id="Pantau pelanggan bawaan, komisi, dan pencairan Anda." en="Track your referred customers, commissions, and payouts." /></p>
