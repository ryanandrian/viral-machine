...
> **Status:** ✅ **SELURUH TEKNIS LIVE PRODUKSI** — F1–F4 + audit A–Z (17-Jul) + perbaikan & MGM §9a (19-Jul 23:28, `a124857`). SPEC diketok owner 2026-07-17; dimatangkan (pajak §6b · kontrak Lampiran A · aturan §5g · inventaris §3b). Sisa = non-teknis owner (§0.1).
...
- ❌ Program referral antar-tenant "ajak teman" (konsep awal) — DIGANTI seluruhnya oleh program agen ini. *(Catatan 19-Jul: MGM §9a.5 BUKAN menghidupkan ini — tenant yang jadi reseller tetap DI BAWAH AGEN & dibayar agen; bukan referral langsung ke kami.)*
...
| **FE-tenant** (panel tenant) | ✅ 1 titik (sejak MGM §9a.5, 19-Jul) | Link nav "Portal Reseller" di sidebar — tampil HANYA bila tenant tertaut reseller (`reseller_linked`); tenant biasa tetap tidak melihat apa pun |
...
3. **Satu email = satu peran** — dengan SATU pengecualian resmi (ketok owner 19-Jul, MGM §9a.5): **email TENANT boleh ditautkan jadi RESELLER pada login yang sama** (penanda `reseller_linked`, bukan role; anti-komisi-diri §9a.2 menjaga uangnya). Agen/admin tetap ketat: email agen ≠ email tenant/admin/reseller — ditolak jelas di titik input.
...
| **Tenant** | Kolom kode saat daftar; **bila tertaut reseller (MGM §9a.5): + portal reseller MILIKNYA** (nav Portal Reseller) | Data program lain apa pun; tenant tak-tertaut tidak melihat apa-apa |
...
- **Anti-kecurangan bawaan:** komisi HANYA dari settlement nyata · atribusi permanen-unik (UNIQUE tenant_id) · **self-referral ditegakkan di MESIN KOMISI (§9a.2, 19-Jul): pembayar = pemilik login agen/reseller → nominalnya di-nol-kan** (dulu hanya tertulis; email-sama juga tertolak alami oleh keunikan email auth) · ambang minimum pencairan · ledger append-only (audit penuh) · agen `suspended` = kode mati seketika tapi ledger utuh.
...
## §9d PANDUAN = TOMBOL HELP (arahan owner: bukan file lepas) — TIGA panduan
...
- **Panduan AGEN** (masuk portal→sebar kode→rekrut reseller→komisi&pencairan+FAQ) = `apps/web/public/panduan/agen.html` → tombol **"Panduan"** di nav **portal agen** (`/agent`). URL publik bisa dikirim ke CALON agen saat rekrut. ✅ LIVE 2026-07-17 22:58 (`d755077`). **✏️ DIREVISI 2026-07-20 (keluhan owner "panduan agen tidak jelas"):** + login Google · + langkah eksplisit "reseller menyebarkan kodenya → pelanggannya otomatis tercatat per-reseller di bawah agen" · + 2 FAQ (calon reseller ber-akun-tenant → ditautkan MGM; bagaimana tahu bawaan reseller mana).
- **Panduan RESELLER (BARU 2026-07-20, ketok owner)** = `apps/web/public/panduan/reseller.html` (style byte-identik satu-set) → tombol **"Panduan"** di topbar **portal reseller** (`reseller-shell.tsx`, pola persis agen). Isi: aktivasi (undangan / akun-tenant-tertaut / Google) · sebar kode+tautan · pencapaian per bulan · **"yang membayar Anda = AGEN Anda"** · FAQ (self-billing tanpa komisi, kode beku, lupa sandi).
- Ketiganya HTML mandiri satu-set (desain sama, tema-aware, nol dependensi), publik tanpa auth (owner: "tidak ada yang rahasia"). File HTML lepas repo-root DIBUANG (anti-fosil §3.2).
- **+ Google login RESELLER (ketok owner 2026-07-20):** tombol "Masuk dengan Google" di `/reseller/login` — cermin persis `/agent/login` (K8 19-Jul), callback `next=/reseller`.
...
- **Fix:** migr **0173** pagar trigger (role agent/reseller → lewati) + `invite/route.ts:32` & `resellers/route.ts:150` set `app_metadata.role` LANGSUNG saat `createUser` (didukung tipe `AdminUserAttributes.app_metadata`).
- **✅ TUNTAS 19-Jul malam (izin owner "jalankan SQL-nya"): 0173 APPLIED (pagar terverifikasi di functiondef) + baris hantu `a3195614…` TERHAPUS (nol sisa hantu partner; agen hilang dari daftar tenant admin).**
...
**REALISASI §9a.5 — ✅ DIBANGUN 2026-07-19 (KETOK owner "KETOK BUILD MGM §9a.5"); tervalidasi lokal, menunggu izin deploy FE.** Terbangun persis desain + 2 pagar TAMBAHAN yang ketemu saat build (di luar desain — kelas bug terpental): (1) middleware isResellerLinked (gate /reseller + /reseller/login; blok pengusir :139 tak tersentuh — linked tanpa role) · (2) guard requireReseller terima linked+baris resellers · (3) approve: email tenant → TAUTKAN (merge manual app_metadata via getUserById — anti-asumsi REPLACE; role tak disentuh; T-5 tetap) + respons `linked_tenant` · (4) email BARU `renderResellerLinkedEmail` dwibahasa (portal aktif, TANPA set-password) dipakai approve & kirim-ulang (kirim-ulang recovery ke tenant = reset password akun tenant-nya — dicegah) · (5) **pagar tambahan:** `reseller/(portal)/layout.tsx:11` & `reseller/login/page.tsx:28` (dua-duanya dulu menolak non-role) · (6) sapu grep repo: nol titik role-reseller tersisa yang buta-tautan. tsc 0 · next build lulus. **✅ DEPLOYED PRODUKSI 19-Jul 23:28 (izin owner "DEPLOY FE + KETOK PINTU MASUK UI"): FE OK `a124857` situs 200 + BE OK worker/webhook active health 200 (log worker bersih pasca-restart). Pintu masuk UI ✅ ikut terbangun (ketok owner): link nav "Portal Reseller" (Handshake, dwibahasa) di sidebar tenant, tampil HANYA bila `reseller_linked` — pola gated persis Niche Studio; tenant biasa nol perubahan.** **✅ BUKTI PRODUKSI NYATA 2026-07-20 09:36 WIB — MGM linking PERTAMA BERHASIL:** agen THETANGGA menyetujui calon "Tresna Wardhana" (tessartea@gmail.com = email ber-akun TENANT existing) → jalur TAUTKAN berjalan persis desain: `reseller_linked=true` terpasang · role TIDAK tersentuh (tetap tenant) · `resellers.user_id` tertaut · kode `RSWKMWVW` aktif · rekening terenkripsi terisi · NOL user baru/hantu. Terverifikasi DB langsung.

## §10 CHANGELOG
- **2026-07-20** — Ketok owner 4 item (keluhan "panduan agen tidak jelas, panduan reseller belum ada" + pertanyaan Google reseller): (1) revisi panduan agen (Google login · atribusi per-reseller eksplisit · FAQ MGM) · (2) **panduan RESELLER baru** + tombol Panduan di shell reseller · (3) **Google login /reseller/login** (cermin agen) · (4) §9d → TIGA panduan. Jawaban terdokumentasi: atribusi reseller = kode unik reseller saat tenant daftar (dua-arah, permanen). **✅ LIVE PRODUKSI 10:25 (`2d798e4`; bukti layar: reseller.html 200 + frasa kunci · tombol Google tampil di /reseller/login · frasa atribusi tampil di agen.html).**
- **2026-07-19 (3)** — **REKONSILIASI SSOT vs codebase (mandat owner "pastikan 100% sesuai")**: 8 titik basi disinkronkan — header status · §0.1 posisi · §1g catatan MGM≠referral-tenant · §3b baris FE-tenant (kini 1 titik nav) · §4 resellers.user_id boleh=tenant · §6 matriks Tenant + kalimat anti-kecurangan (self-referral kini ditegakkan mesin komisi) · catatan F3 realisasi. Validasi nol-bug pasca-deploy TERBUKTI: trigger dua-fungsi utuh (pagar + insert tenant normal) · nol hantu · 6 halaman produksi benar (200/307) · log mv-web & worker bersih · suite BE 42/42.
- **2026-07-19 (2)** — **SEMUA §9a LIVE PRODUKSI 23:28** (`51f8384`+`ed7bf67`+`a124857`; FE OK 200 · BE OK health 200 · log worker bersih): SQL 0173 applied + hantu terhapus · anti-komisi-diri · Google login agen (bukti layar: tombol tampil di /agent/login produksi) · pagar email-PIC · **MGM §9a.5 DIBANGUN+DEPLOYED** (ketok "KETOK BUILD MGM §9a.5" + "DEPLOY FE + KETOK PINTU MASUK UI") termasuk link nav Portal Reseller gated. §5g.3 diperbarui (pengecualian resmi reseller⇄tenant satu login).
