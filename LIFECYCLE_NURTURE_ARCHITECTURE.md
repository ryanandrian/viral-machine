...
📦 **SISAKAN (legal/anti-abuse, anonimkan bila perlu):** `payments` (bukti bayar/legal), `tenant_configs` (set `subscription_status='deleted'` + strip PII), `feedback_submissions` (insight lead — anonimkan email), **`commission_ledger`** (AGENT §5g.8 "seluruh ledger/payout DISIMPAN — kewajiban audit & pajak"; catatan uang ke PIHAK KETIGA/agen, bukan data tenant), **`tenant_attribution`** (AGENT §1.3/§2 atribusi "terkunci PERMANEN"/"selamanya" + "Masa komisi: SELAMANYA"; **dihapus = agen kehilangan komisi selamanya bila tenant kembali**, sebab akun login tenant TIDAK ikut dihapus dan atribusi hanya ditulis saat pendaftaran). **`youtube_channel_claims`** (CHANNEL_LOCK_ACTIVATION_PLAN §7 — klaim channel YouTube ADA justru untuk bertahan melewati penghapusan; dihapus di sini = kuncian anti-masa-coba-berulang lenyap lewat jalur termudah bagi penyalahguna: **hapus akun → daftar baru → sambung channel yang sama**. PDP dipenuhi tanpa membuka lubang: baris DISIMPAN, `tenant_id` **dianonimkan** ke `__dihapus__` ⇒ nol pengenal tenant tersisa, channel tetap terkunci, pelepasan hanya lewat admin — ditambah 20-Agu).
