...
- **🔴 ADMIN nol role-gating (SECURITY):** `/admin/*` cuma cek session (middleware) — **TANPA cek super-admin** → **tenant biasa bisa buka admin penuh**. Label "Super Admin only" di Pricing = kosmetik. → **nuansa lintas-area #2 = prasyarat keamanan**, wajib sebelum admin go-live. Write-action admin: suspend/credit/impersonate · resolve tiket · inline-edit pricing · edit niche 6-tab · toggle model/bahasa · transition exclusivity (semua mock).
...
- **[Area 2.1] C1-C5 Onboarding** (signup→**BYOK keys**→YouTube OAuth→niche+bahasa+voice→jadwal). **Trial wajib BYOK upfront** (hapus "skip keys"). Tulis `tenant_configs`/`channels`/`tenant_credentials`. ⚠️ **`tenant_credentials` lewat server-route** (nuansa lintas-area #1) + tambah write-policy per-tabel (#3).
  ...
  - ⏳ **Increment 2 (next):** server-route whitelist → `tenant_configs` (AI keys C3, voice, timezone) + `tenant_credentials` (OAuth C2, Fernet) · production_schedules slots (C5) · brand_color (FE pilih warna, **belum ada kolom DB — keputusan: tambah `channels.brand_color`? flag**). YouTube OAuth = **gate owner** (Google config / BYO-CC client).
