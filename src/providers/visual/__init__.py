...
inline tersebar di visual_assembler). Family diturunkan dari `visual_mode`:
  • 'video'        → PexelsProvider (stock footage)
...
def _registry() -> dict:
    ...
    return {"pexels": PexelsProvider, "ai_image": AIImageProvider, "ai_video": AIVideoProvider}
...
def build_visual_provider(visual_mode: str, config: dict) -> VisualProvider:
    ...
    if not cls:
        raise VisualError(
            f"visual_mode '{visual_mode}' tak dikenal. Gunakan 'video' | 'ai_image:<model>' | 'ai_video:<model>'."
        )
    return cls(config)
