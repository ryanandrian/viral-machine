...
- **Dua opsi akun yang sah (koreksi owner 2026-07-14 — revoke BUKAN kematian permanen, sistem pulih via reconnect, terbukti 06-27 & 07-01):**
  ...
  - **✅ MEKANISME RECONNECT DIVERIFIKASI 3-LAPIS (2026-07-14, mandat owner "pastikan, nol asumsi"):** (1) **Kode** `youtube_oauth.py`: `prompt="consent"`+`access_type="offline"` → Google WAJIB kirim refresh_token baru tiap reconnect; DEDUP `_find_existing_connection` → channel yang sama di-consent ulang = **token disegarkan ke BARIS LAMA yang sama** (pesan "already → token disegarkan") → `channels.youtube_account_id` TAK berubah, produksi langsung sehat TANPA repoint; tanpa refresh_token/identitas → gagal jujur. (2) **DB live**: kedua koneksi ryan `status=valid`, scopes 3/3, channel menunjuk baris benar. (3) **Runtime**: log worker 13-Jul 19:57 token dipakai sukses (Data API + Analytics v2 full metrics, sync meta RAD subs=181) + bukti historis reconnect nyata 06-27 (publish pulih) & re-consent 07-08 (kedua identitas). Catatan: bila baris koneksi DIHAPUS di app sebelum reconnect → baris BARU dibuat & channel perlu di-repoint via picker (jalur ini pun terbukti 06-27) — utk skenario demo cukup revoke di Google TANPA hapus baris di app = pulih otomatis nol-repoint.
...
- **⚠️ Tenant demo daftar pakai EMAIL+PASSWORD, BUKAN tombol Google (SSO)** — SSO membuat akun Google memberi izin dasar duluan → consent YouTube berisiko tampil "additional access" lagi. Bila akun Google-nya pernah SSO/connect ke mesinviral → cabut dulu di `myaccount.google.com/permissions` (aman utk akun non-produksi). *(Konteks: 2 tenant uji rw23mutiara + oauthtestcs sudah dihapus BERSIH total 2026-07-14 — DB+auth; grant sisi Google dicabut manual oleh owner.)*
