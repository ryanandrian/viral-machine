...
> **🔵 PER-CHANNEL OAUTH (epic multi-channel — Pro 3/Business 10 channel; 1 user=1 tenant TAPI 1 tenant MULTI channel) — Fase 1-3 DONE+deployed, 4-6 SISA.** Akar gap: `tenant_credentials` PK=tenant_id → cuma 1 channel YouTube/tenant, padahal tier jual multi-channel. Pendekatan: tabel BARU **`channel_credentials`** (PK channels.id FK, service_role RLS) + **fallback `tenant_credentials`** (ryan/produksi lama TAK putus). **✅ Fase 1** (migr `0060`): channel_credentials + backfill ryan (channel_id=410d4538, refresh cocok). **✅ Fase 2** (`7434301`): `load_google_credentials(tenant_id, channel_id=None)` prefer channel_credentials→fallback tenant; `save_google_access_token(...channel_id)`. VALIDATED (per-channel+fallback+konsisten+ch-tak-dikenal). **✅ Fase 3** (`b290bee`, deployed mv-worker): `youtube_publisher._get_credentials(channel_id)` pakai `tc.channel_id`; `channel_analytics` creds per-channel + **`sync_channel_meta(channel_id)` SCOPE ke channels.id** (fix multi-channel name-overwrite); `self_learning` per-channel. VALIDATED: ChannelAnalytics(channel_id)→channel_credentials→sync "RAD The Explorer"/164. **✅ Fase 4** (`66f4e7d`): `youtube_oauth`+`webhook_app` per-channel — sign/verify_state bawa channel_id; save_client_creds/_store_tokens/connection_status/disconnect baca-tulis `channel_credentials` bila channel_id (else tenant_credentials legacy); handle_callback ambil channel_id dari state + simpan yt_channel_id; webhook_app endpoints oper channel_id. VALIDATED (state roundtrip t=RY/c=CH; status per-channel connected=True; ch-belum-connect=False). **✅ Fase 5** (`976a4bb`, FE deployed): 3 route `/api/youtube/*` oper channel_id + **kartu "Koneksi YouTube" per-channel di `/channels/[id]`** (form client_id/secret→connect channel_id+ret=/channels/[id]; status; disconnect; tangani ?youtube=connected|error; degradasi anggun bila vault belum jalan). **AUDIT function/trigger Supabase: NOL terdampak** (tak ada fungsi DB sentuh credentials; trigger handle_new_tenant tak terkait; FK channel_credentials→CASCADE on channel-delete = bersih). **✅ Fase 6 DEPLOYED & VALIDATED:** `webhook_app` (FastAPI) jalan sbg systemd **`mv-webhook`** (uvicorn `127.0.0.1:8088`, EnvironmentFile=worker .env) + **nginx** route `https://mesinviral.com/api/youtube/oauth/*` → :8088 (callback publik 302 ✓) + env publik (`YOUTUBE_OAUTH_REDIRECT_URI`/`APP_BASE_URL`/`NEXT_PUBLIC_YT_REDIRECT_URI` = `https://mesinviral.com/...`) + mv-web rebuilt. Rantai penuh VALIDATED: status ryan via webhook_app = connected:true; ch-belum-connect=false; tanpa secret=401. **PER-CHANNEL OAUTH EPIC SELESAI (Fase 1-6).** **SISA (aksi OWNER, B2-eksternal):** daftarkan redirect URI `https://mesinviral.com/api/youtube/oauth/callback` di **Authorized redirect URIs OAuth app YouTube tenant** (ryan=`963179529813-vikbs304...`; BUKAN app login platform `153190...`) → lalu uji e2e connect channel baru via /channels/[id]→Settings→Koneksi YouTube. ryan channel existing sudah connected (tak perlu aksi). Semua backward-compatible. **Catatan: `MV_INTERNAL_SECRET` worker==mv-web (verified); credentials tetap service_role-only.** **Tabel lain (content_inventory/production_runs dll) channel_id TEXT-nullable TIDAK disentuh** — tak relevan OAuth, hindari risiko (proyek data-hygiene terpisah).
> **✅ SELESAI 06-19 (DASHBOARD D1 — rombak KPI data-nyata + AppShell + auto-refresh, DEPLOYED `25cfae9` + migr `0056`):** 4 KPI profesional tenant-wide: **Success Rate** (all-time `production_runs`, `qc_failed` dipisah "perlu ditinjau"; ryan 94% = 104/97/6/1) · **Total Views/Likes/Followers YouTube** via RPC `get_tenant_youtube_totals` (SECURITY DEFINER, RLS `auth.uid()`; views/likes = **snapshot-terbaru/video** bukan sum mentah; followers = `channels.subscriber_count` BARU). ryan: Views 34.762 · Likes 1.006 · Followers 162. **Jadwal Hari Ini** kini baca `channels.publish_slots` (zona tenant) — dulu hardcoded kosong. **Auto-refresh SMOOTH**: realtime `production_runs` (debounce 400ms) + re-fetch saat tab aktif, tanpa spinner. **AppShell**: sb-tenant data nyata (display_handle/plan/kanal+inisial), buang "Ganti tenant"+badge"3"; **hapus search+lonceng**; hapus tombol Bantuan bawah (duplikat). **BE**: `ChannelAnalytics.fetch_subscriber_count` (fail-soft, `mine=True`) di loop self_learning 24j. **NOL kode khusus-ryan** — semua tenant-scoped (RLS/per-tenant); v1 ryan terhitung otomatis krn datanya sudah di tabel tenant-nya (tak menyentuh tenant lain). Cadence data: production_runs **live** (FE refresh realtime/fokus); Views/Likes/Followers/Compliance **tiap 24j** (`SELF_LEARNING_INTERVAL_SEC`). **+ Follow-up 3 kartu (deployed `d901a1b`, migr `0057`):** Compliance kini **AGREGAT lintas-channel** (RPC `get_tenant_insights_summary` — insight TERBARU per channel NYATA, `channel_id IN channels tenant` → cegah double-count orphan/legacy v1; formula GENERIK semua tenant, bukan patch ryan) · Self-Learning **di-wire nyata** (video dipelajari + niche teratas + terakhir belajar) · Biaya AI → **"Coming soon"** (worker belum catat cost). **+ NAMA CHANNEL = YouTube (deployed `cfe39c4`):** `sync_channel_meta` (ganti `fetch_subscriber_count`) — `mine=True` → `channels.channel_name`=judul YouTube (WAJIB sama, anti-confuse) + `platform_channel_id` + `subscriber_count`. GENERIK semua tenant (sumber kebenaran nama = YouTube, bukan niche/input), fail-soft, di loop self_learning 24j. ryan diperbaiki: "Ryan Andrian — Universe & Mysteries" (salah) → **"RAD The Explorer"** (UCo5d8bH2MnNdIuwItgPtJ6Q). *(Catatan: onboarding `finish()` masih set channel_name dari niche = placeholder s/d YT connect → sync overwrite; rapikan saat rombak onboarding. Orphan `channel_insights.channel_id='ryan_andrian'` v1 = bersihkan saat rombak `/compliance`.)* **✅ `/analytics` SELESAI (deployed `98b2ff8`, migr `0058`):** ternyata sudah real (label "MOCK" keliru) → **dibangun ulang kelas-pro** via 5 RPC server-side (overview/by-niche/monthly/top-videos/learning; tenant-scoped, agregat efisien, snapshot-terbaru/video). KPI Followers/Views(+30hr)/**Retensi**(+n video ber-data)/**Engagement**/Video/Komentar · Performa per-niche · **Hook & Topik teratas** (MOAT self-learning) · tabel Video teratas sortable. **CTR DIBUANG** (tak tersedia YouTube API). ryan: 158 video, retensi 89,9%, engagement 3,10%. **+ `/runs` & run-detail dibenahi (deployed `2884e55`, migr `0059`):** kolom Views di-wire (RPC `get_tenant_video_views`; sumber video_analytics) · channel_id backfill historis ryan (99 baris→channel nyata) · topic dari YT title · "Durasi"→"Proses"/"Waktu proses" (jelas=waktu proses, bukan durasi konten) · titik-3 mati→ikon Detail (link /runs/[id]) · log query utamakan run_id · kartu Rincian biaya→"Coming soon" · dedup top_hooks/top_topics di /analytics (analyzer hasilkan duplikat). **+ FETCH-ANALYTICS coverage fix (`88db9d9`):** `_get_videos_to_fetch` dulu hanya 50 video TERBARU → video lama (views tinggi) snapshot beku berbulan (April) → views tak cocok YouTube. Fix: rotasi **stalest-first** (semua video ter-refresh giliran). Refresh paksa ryan → views per-video kini **COCOK PERSIS YouTube** (terverifikasi live: 1910=1910). Sisa: total channel YT 38.331/175video vs kita 35.046/158video (17 video belum ter-track ~8%). **+ /runs paging 25/hal (load semua, `236e299`) + search box + filter channel/waktu/niche fungsional.** **+ BUG FIX publisher (`b644df0`): jalur buffer/terjadwal TAK menyimpan `video_id` ke `production_runs`** (hanya ke tabel `videos`) → Views "—" untuk run terjadwal. Fix: `_publish_from_buffer` update `production_runs(youtube_video_id,youtube_url,status)` via run_id pasca-publish → run V2 ke depan PASTI ter-link. Backfill historis ryan: via `videos.run_id` (2) + waktu-produksi 1:1 (2); 24 run v1 tak-terlink (linkage hilang migrasi, "—" jujur, tak ditebak). **+ ✅ LOG BUFFER-RUN FIX (`1d1590c`):** `produce_one` kini `logger.contextualize(tenant_id,channel_id,run_id)` bungkus pipeline + oper `run_id` ke `Pipeline.run(run_id=...)` (param baru, backward-compat) → log buffer/terjadwal TERSIMPAN ke `pipeline_run_logs` (ber-run_id cocok production_runs); FE detail utamakan run_id. **+ retensi**: `buffer_janitor.prune_logs` hapus log >`LOG_RETENTION_DAYS`(30) tiap siklus (anti-bloat). Diverifikasi load-clean di VPS (signature backward-compat). **+ ✅ `/channels/new` (`2d58807`): filter niche per-entitlement** (cegah bocor niche eksklusif tenant lain/di luar paket; logika identik channel-detail). **+ ✅ tombol YouTube Studio** di channel-detail (`ad63ce3`). **BERIKUTNYA (owner minta, urut):** (1) benahi page **`/compliance`** (MOCK) + treatment multi-channel + bersihkan orphan `ryan_andrian` · (2) **BE cost-tracking** (token×harga per run → kartu Biaya AI nyata) · (3) **BE dedup `top_hooks`/`top_topics`** di `performance_analyzer` · (4) `/channels/new` polish layout (#b, tunggu spesifik owner) · (5) sync 17 video belum ter-track · KPI header channel-detail (data sudah ada). 
...
- **✅ PHASE 6.4 (parsial, terverifikasi):** insights **per-channel** *(koreksi 2026-06-28: compute dulu se-tenant → bleed antar-channel; KINI benar per-channel via `_fetch_analytics` filter channel_id, commit `3bd32ee`, deployed — [[project_self_learning_remediation_2026_06_28]])*. **per-tag DEFERRED** (terblokir: `videos.topic_tags` belum ada + butuh sistem Layer-2 tag [tag_pool+assign] + histori tag → prematur; sequence dgn niche-UI P9-10). Keputusan expert, terdokumentasi.
...
- [~] **D5 — F4 Self-improvement** *(loop inti LIVE 2026-06-28):* `channel_insights` + `historical_factor` jalan; **`viral_score_weights` (S3-A) KINI benar-benar hidup** — dulu penulisnya cron v1 yg TAK PERNAH ter-wire di v2 (bobot stuck baseline); kini di-wire ke `self_learning` worker (commit `21f41fe`, deployed; ryan n=130, alpha=1.0, evergreen_potential dominan). Detail [[project_self_learning_remediation_2026_06_28]]. **Sisa (roadmap/data-gated):** kalibrasi `source_weights` dari outcome + panen sinyal Analytics kaya (trafficSource/retensi/CTR — `channel_analytics` sebagian sudah) → butuh **akumulasi analytics nyata** (post-cutover, nyambung §GATE CUTOVER E3). Mekanisme siap, aktivasi = data.
...
**FASE 2 — Cacat B (akurasi durasi → lolos QC):** ✅ **SELESAI + DEPLOYED (audit 2026-07-01).** Diselesaikan tuntas di **REMEDIASI F4-02/03/04 = durasi-via-speed** (commit `8670fc3` + migr 0078/0079; LLM pilih kata+speed + per-beat word-budget absolut + gate durasi). Validated LLM-only **9/9 preset first-pass** (incl. 15s/30s) + render nyata QC PASS. **Menggantikan B2 di bawah.** ⛔[dicabut 31-Jul → §2c]
- [x] B1 — **budget speed-adjust** (`detik × delivery_wps × niche_speed`). → 45/60/75/90 lolos (pendahulu; kini disempurnakan durasi-via-speed F4). ⛔[dicabut 31-Jul → §2c]
- [x] B2 — ✅ **DITUTUP via F4 durasi-via-speed** (per-beat word-cap absolut + speed-lever multi-provider + gate durasi). 15s/30s mendarat di window (validated). *(Sisa poles = F5-01 closed-loop kalibrasi, data-gated.)* ⛔[dicabut 31-Jul → §2c]
...
- [x] F4 — **Buang fosil pexels TUNTAS A-to-Z (V2 = generator AI saja; keputusan owner 2026-06-24: Pexels = LIBRARY stok, BUKAN generator).** Dihapus: `pexels.py`, family/registry pexels (`providers/visual/__init__.py`), `_try_pexels`+cabang `'video'`+BLACK_SCREEN (`visual_assembler.py`), default `visual_mode`/`visual_provider` pexels + field `visual_fallback_mode` (`tenant_config.py`), pesan error ai_video/base, toggle footage FE (Visual=generator gambar/video, ai_models image+video). `PEXELS_API_KEY` dibuang dari `.env`. Migrasi **0088_purge_pexels_footage** (scrub `'video'`/`'pexels'` → null + drop kolom `visual_fallback_mode`). Visual=`ai_image:`/`ai_video:` saja (Seedance/Veo3 dst = tambah baris katalog). Validasi: py_compile+import+tsc+rollback DB PASS; ryan (ai_image) tak tersentuh. Deploy = batch bersama 0089 (kredensial). *(2026-06-24)*
