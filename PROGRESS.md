...
| **4** | BYO-CC Phase 1 | `tenant_credentials` + Fernet + auth foundation | 1 minggu | 🛠️ in-progress — **4.1✅** crypto+creds · **4.2✅** Auth user+tenant_id→UUID · **4.3✅** RLS go-live+security fix (2026-06-13); 4.4/4.5 pending. §PHASE 4. |
...
> - **⏳ 4.4 (OAuth file→`tenant_credentials` decrypt + refresh-to-DB)** = code DB-first + file-fallback; seed token ryan (VPS v1) utk e2e.
> - **⏳ 4.5 (mandatory key validation + hapus fallback)** = code (fail-loud per provider).
