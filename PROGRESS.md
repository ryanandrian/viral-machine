...
| Multi-Format | `format_profiles`+`duration_presets`+`tts_profiles`+branded(0015) ✅ (0012-0015) | **F1+F2 ✅** + **Branded FB1-3 ✅** (link deskripsi + soft-sell CTA); **FB4 logo-overlay PENDING** (renderer inti, validasi lokal, butuh upload-UI P9) | **screen baru BELUM ada** (Hybrid §11) | DB ✅ · BE ✅ (FB4 pending) · **FE gap Phase 9-10 (WAJIB): (a) format/preset picker; (b) Branded Content panel — link + soft-sell + upload logo, SEMUA OPSIONAL/toggle (backend nullable=non-breaking)** · closed-loop pasca-ElevenLabs |
