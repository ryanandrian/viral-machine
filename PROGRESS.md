...
- [ ] **D2 — F1 Decouple + cache (kritikal scaling 429, NOL kredit AI):**
  - [ ] **F1a** — migr `trend_cache` (cache_key niche+geo+source+timeframe, signals jsonb, fetched_at, ttl_sec) + RLS service-role/public-read; **update `DB_SCHEMA_V2.md`** (aturan owner).
  - [ ] **F1b** — `TrendRefresher` thread di `worker_decoupled` (ambil (niche,geo) aktif dari channels → fetch sumber utk cache basi, pacing config `app_config`, TTL config) — di luar hot-path.
  - [ ] **F1c** — produce/`trend_radar.scan` **baca `trend_cache`** (fresh), TIDAK panggil sumber langsung; resilient (cache basi → pakai terakhir). Hot-path nol-fetch-eksternal.
  - [ ] **F1d** — validasi: produce nol-429 (nol fetch eksternal) · refresher tulis cache · konstan vs jumlah tenant.
