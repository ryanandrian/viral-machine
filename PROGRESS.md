...
| **5** | Multi-Channel + Decouple | `channels`, channel_id, content_inventory, S3 buffer, producer/publisher | 1 minggu | 🛠️ in-progress — 5.1 ✅ + **5.3 decouple CODE-COMPLETE** (inventory+producer+semaphore-brake+publisher+timezone-slot=**Bug1 fix**, semua tervalidasi). **Step 4 cutover = GATED run-nyata**. `PHASE5_DESIGN.md`. |
