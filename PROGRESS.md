...
>
> **BERIKUTNYA (PR terbuka, urut):** (1) verifikasi atempo+auto-publish slot 21:00 · (2) label run utk item buffer yg sudah hilang (run `qc_failed` lama, S3 disapu janitor) · (3) buang fosil **pexels** (kolom legacy `tenant_configs.visual_provider`; efektif=`visual_mode`) · (4) §OPSI-C F2/F3 + C1/C2 publisher reporting. Ditunda: 8s ai_video, swap VPS, koordinasi niche antar-batch (kalau scale paralel).
