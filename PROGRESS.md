...
**🎯 THREAD AKTIF (2026-06-14) = PHASE 6 (CORE MOAT). 6.1 ✅ → berikutnya 6.2 diversity engine.** Frontend track ✅ DITUTUP (28 screen mock). Branch kerja `v2-backend` (~52 commit, pushed); `main` aman (v1). Migrasi v2 ter-apply: **0001–0017**.
...
- **Gate owner:** cutover (deploy worker_decoupled ke VPS + flip publik) = operasional · **bayar ulang ElevenLabs** (lapse → free) untuk kalibrasi `tts_profiles.delivery_wps` nyata + closed-loop + voice premium.
...
| **8** | Payment Integration | **Midtrans** Snap + Email (SMTP) + tier-gating | 2 minggu | ✅ **BACKEND DONE (2026-06-14)** — 8a tier-gating+subscription-gate · 8b Snap-redirect+webhook+payments(0022)+comp · renewal/grace/suspend · trial BYOK-tier+trial_expired-leads · 8c email SMTP+auth-admin-resolve+3 notif. e2e sandbox NYATA penuh. **Sisa = CUTOVER ops** (owner: 6 URL dashboard, prod key, deploy webhook_app). |
| **9** | UI Foundation | Next.js + landing + dashboard + Supabase Realtime + RLS | 4-6 minggu | 🟢 **UNBLOCKED (Phase 8 done)** — wiring 28 layar mock → Supabase v2. **Breakdown: [`PHASE9_FRONTEND_WIRING.md`](PHASE9_FRONTEND_WIRING.md)** (9.1 fondasi→slice→beta-path). |
| **10** | UI Polish | Onboarding wizard + admin (E1-E5) | 2-3 minggu | 🔒 setelah 9 — detail di [`PHASE9_FRONTEND_WIRING.md`](PHASE9_FRONTEND_WIRING.md) §10 (Admin/Trial-Leads + polish/next-intl/PWA). |
