...
| **8** | Payment Integration | **Midtrans** (akun owner sudah ada) webhook handler + Email (Resend) + tier-gating | 2 minggu | 🛠️ **inti DONE (2026-06-14)** — 8a tier-gating+subscription-gate · 8b Snap-redirect+webhook+payments(0022)+comp-account · renewal/grace/suspend sweep. e2e sandbox NYATA (checkout→aktivasi). Sisa: 8c email Resend (gate akun owner) + cutover (prod key+deploy webhook). |
