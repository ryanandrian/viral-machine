...
| 2.2 | `viral_score_weights` per-channel + seed warisan | 🔀 **PREMIS GUGUR → DILEBUR ke B17 §6 (ketok owner K1–K6, 2026-07-18).** Deep-dive empiris 18-Jul: fitur `topic_scores` (tebakan LLM) = HAMPA informasi — korelasi vs hasil nyata \|r\|≤0,24 (n=130 RAD), dalam-1-niche pun nol, bobot live rata 0,2 → per-channel-isasi lapisan ini SIA-SIA di granularitas mana pun. Jalan benar = ganti fitur dgn atribut nyata (kurva retensi per-momen, Lapis 1) + otak analis ber-buku-keputusan (Lapis 2). "Seed warisan" terwujud di Lapis 3. **Sumber kebenaran = `PROGRAM_BUKTI_KECERDASAN.md` §6 — jangan kerjakan item ini dlm bentuk lama.** | | Bukti lengkap = B17 §6b |
