...
| G5 | **invalid_grant senyap**: tak ada penandaan `status='invalid'`/nonaktif/notif saat refresh-token mati (grep nihil di distribution/analytics/orchestrator; publisher hanya `failed` per-run `youtube_publisher.py:304-306`). | Melanggar "no silent degradation"; berlaku juga single-channel. |
