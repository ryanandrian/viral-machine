...
> - 🔴 **INSIDEN 18/19-Agu — LARANGAN DI BARIS ATAS DILANGGAR, 8 PRODUKSI TENANT TERBUANG.** Baris ROOT-CAUSE di atas sudah menulis **"JANGAN dikalibrasi ulang membuta"**, dan Claude tetap menulis `voice_catalog.delivery_wps` + `tts_profiles.delivery_wps` **dari SATU teks uji 32 kata** saat menyalakan suara Gemini. Produksi memakai 120–207 kata. Akibat terukur: **Kore 1,93 vs nyata 2,38 (+23%) · Aoede 2,05 vs 2,67 (+30%)** ⇒ resep naskah menuntut terlalu sedikit kata ⇒ **8 dari 8 video "kependekan"** (nol "kepanjangan") ⇒ QC menolak. **$2,30 ≈ Rp 37.956 uang tenant**, 4 channel / 3 tenant. Cara sah yang dilewati: `scripts/ukur_jeda_suara.py` **atau** kalibrasi dari `tts_delivery_samples` — keduanya sudah ada sejak F2/F5.
>   **DIPERBAIKI 19/20-Agu** dari sampel produksi yang mesin kumpulkan sendiri (Aoede 8 · Kore 3): pace **dasar** Gemini **1,91 → 2,59** (median 11 produksi) · **Kore 2,38** · **Aoede 2,66** · **Puck & Charon DIKOSONGKAN** (nol sampel ⇒ jatuh ke pace dasar terukur, bukan angka karangan). Ditaruh di lapis yang BENAR menurut rantai-baca F2: *terkalibrasi(voice×niche→'\*') → `voice_catalog` → `tts_profiles`*.
>   **⚠️ SISA YANG DISEBUT TERANG (sesuai peringatan baris ROOT-CAUSE):** angka itu **satu per SUARA**, sedangkan rancangan menuntut per **(suara × niche)** — sampel Aoede sendiri tersebar **2,34–2,80 wps** lintas 6 niche (spread ±20%, konsisten dgn temuan "s/d 25%"). Presisi per-niche datang dari `pace_calibration` begitu sel mencapai **14 sampel** (`min_n`); Kore baru 3, Aoede 8. **Sampai itu, taksiran per-niche masih bisa meleset ±12% — bukan bug baru, tapi batas yang diketahui.**
>   **PENJAGA (supaya kelas ini tak bisa terulang tanpa ditolak mesin):** `tests/test_pace_suara_selaras_produksi.py` mengikat `voice_catalog.delivery_wps` **dan** `tts_profiles.delivery_wps` ke **median sampel produksi** (toleransi ±15%, min 3 sampel). Menulis angka "membuta" lagi ⇒ **uji MERAH**, commit ditolak.
...
5. **STATUS di FE (penting — beda sumber):**
   - **Runs/Produksi** = ledger `production_runs` (TERMINAL): `success`→Completed · **`qc_failed`→"Ada catatan QC"** · `discarded`→Dibuang · `failed`→Failed. (Bukan live/FIFO. Tak ada state "running"/"queued" di production_runs.)
   ...
   - **⚖️ PEMISAHAN NAMA — DIPATRI 2026-08-05 (aturan di atas dilanggar layar selama berbulan-bulan):** nama **"Perlu Ditinjau"** MILIK ANTREAN saja (menu + halaman `/review`, satu-satunya tempat ada tombol Pakai/Buang). Buku-besar memakai **"Ada catatan QC"** di SELURUH permukaannya: lencana & tab `runs-table.tsx` · lencana besar `runs/[id]` · rincian Success Rate Beranda · KPI Analytics channel. **Boleh menyebut nama antrean HANYA di dalam cabang yang sudah membuktikan barisnya ada** (`punyaItemTinjau` di `runs/[id]`). Dijaga uji `tests/test_label_tinjau_tak_bohong.py` (11 uji, dua arah: lampu-palsu TIDAK lahir lagi **dan** nama antrean tak boleh ikut terhapus).
     **Kenapa dipatri:** jalur `test`/`retry` mengunggah video PRIVAT ke YouTube tenant (perilaku ber-izin owner sejak 16-Jun, `PROGRESS.md` A4) dan **TIDAK PERNAH membuat baris `content_inventory`** ⇒ ketiga penutup di §7.3/§7.4 (approve · discard · TTL janitor) **tak satu pun bisa menyentuhnya** ⇒ `qc_failed` abadi. Terukur di DB live 05-Agu: **8 run, 5 tenant, terlama 4 minggu**, semuanya memakai lencana "Perlu Ditinjau" untuk pekerjaan yang **tak pernah ada di aplikasi kita** (keputusan publik/hapus terjadi di YouTube Studio), dan halaman detail run menawarkan tombol "Tinjau" → **halaman kosong**. **Bukan penutup baru yang dibutuhkan, melainkan nama yang jujur** — nol kolom baru, nol RPC, nol migrasi, nol pekerjaan untuk tenant, nol sentuhan mesin.
     **SUMBER ANGKA buku-besar WAJIB tetap `production_runs`** (rincian `dibuat = sukses + gagal + ber-catatan + dibuang` harus berjumlah benar — diverifikasi 05-Agu: 232+79+9+9 = 329 ✅). Angka menu (antrean=1) memang BOLEH berbeda dari angka tab (buku-besar=9); yang dulu menyesatkan adalah **namanya sama**, bukan angkanya beda.
     **Sisa yang SENGAJA tak disentuh:** cabang ketiga `ReviewVenue` ("kedaluwarsa — dibuang otomatis (TTL)") bisa keliru bagi run **Test Niche** (`test_nopub` — `producer.py:489` bisa melahirkan `qc_failed` tanpa URL YouTube, artefaknya berstatus `test` di gudang). **Nol baris seperti itu ada di DB per 05-Agu**, jadi kalimat penggantinya akan jadi karangan tanpa kasus nyata. Dicatat sebagai lubang laten, bukan diperbaiki dengan tebakan.
...
### Changelog
- **2026-08-02 (3) — EMPAT CACAT LAGI, SEMUANYA TERTANGKAP OLEH MENJALANKAN YANG SEBENARNYA (§2c).** Bukan satu pun ditemukan dengan membaca kode. (1) **Kerja penulis hangus:** pipeline nyata BISIK NUSANTARA — enam dari tujuh adegan sudah berhasil ditulis ulang (76→149 kata), adegan ke-7 kena kuota harian Groq, SELURUHNYA dibuang, naskah kembali 76 kata (29 dtk untuk preset 90). Naskah asal selalu punya teks tiap adegan (syarat A2), jadi kini yang gagal ditambal teks asalnya — bagian yang berhasil tetap dipakai. (2) **Narasi satu tarikan napas:** llama-3.3 mengirim 76 kata dengan NOL titik, dan penyebabnya UMPAN BALIK KITA SENDIRI ("merge sentences instead of adding them") yang ditelan mentah model lemah. Instruksi diberi batas bawah + cacat PARAH baru `narasi_tanpa_jeda` (kepadatan kata/kalimat, kenop `script_maks_kata_per_kalimat`). (3) **Pemeriksa menuduh hantu:** elipsis ikut cocok pada pola "titik ganda" & "huruf kecil setelah titik" → 15 temuan palsu pada 82 naskah, dan penulis diminta memperbaiki sisa-penggabungan yang tak pernah ada. (4) **Berhenti di bibir band:** BJ Yusroon mendarat 82,1 dtk pada band 82,5–97,5 — mesin berhenti memperbaiki saat ramalannya menyentuh tepi, padahal galat ramalan ±1–2 dtk. Sasaran PENULIS kini dijauhkan 10% lebar band (kenop `script_margin_band_pct`); GERBANG tetap memakai band asli (diuji: nol video sah ditolak). **Hasil rantai 6 channel: 4/6 → 5/6 MASUK band, ramalan meleset rata 3,44 → 2,61 dtk**; BJ Yusroon 82,1 → 86,1 dtk. Sisa satu = RAD (ElevenLabs) menunggu 0187. **547 uji lulus.** Nol migrasi baru (dua kenop menumpang 0188 yang belum diterapkan).
- **2026-08-02 (2) — GAMBAR MELESET SAMPAI 7 DETIK DARI KATA YANG DIUCAPKAN (§2c).** Cacat SENYAP yang menyerang issue utama #2. `compute_beat_durations` menyusuri penanda waktu sebanyak jumlah kata BEAT AKTIF, padahal audio dibuat dari `full_script` yang memuat lebih banyak — model mengisi beat yang tak dipakai preset lalu menulis ulang sambungannya. **Diukur pada 82 video produksi:** 47 punya `full_script` 9–43 kata lebih panjang dari gabungan beat aktifnya · 80 kejadian beat tak-aktif berisi teks · 22 video punya `core_facts_2` berisi 18–32 kata (kunci yang tak dikenal katalog, dan cabang pengaktifnya kode mati) · kemiripan kata median 0,86. Durasi TOTAL tetap benar karena dinormalisasi → **nol alarm, nol QC, nol Telegram** yang menangkapnya. **Perbaikan:** tiap beat dicocokkan ke deretan kata audio; batas adegan = waktu kata pertamanya; kata yang tak dimiliki beat mana pun jatuh ke adegan sebelumnya (memang benar begitu). Gagal cocok → jalur lama (nol regresi). **Bukti runtime, 4 naskah produksi dirender ulang:** meleset rata 3,31/1,67/1,46/1,94 → **0,07/0,01/0,02/0,02 dtk**; terburuk 7,13/4,51/2,52/6,79 → **0,49/0,10/0,10/0,10 dtk**. **Akar hulu ikut ditutup:** `core_facts_2` dicabut dari skema JSON (meminta kunci yang tak pernah dipakai = undangan mengisinya), dan teks yang terlanjur nyasar DILIPAT ke adegan sebelumnya — bukan dibuang (nol kata hilang, urutan bicara identik; sebelumnya teks itu tak dimiliki adegan mana pun sehingga jatah kata menghitung 127 padahal 170 terucap, dan bisa HILANG total bila model tak mengirim `full_script`). Hilir diperiksa: perakit visual & renderer hanya menskala proporsional, Ken Burns sudah meng-clamp travel → adegan panjang tak over-zoom. **518 uji lulus** (+13). Nol migrasi · belum deploy.
...
- **2026-08-22 — PESAN "VIDEO MENUNGGU KEPUTUSAN" TAK PERNAH SAMPAI PADA JALUR TERJADWAL (dilaporkan owner).**
  `notify_review_pending` mengambil nomor Telegram **hanya** dari lembar setelan yang diserahkan pemanggil.
  Satu-satunya pemanggilnya — `producer.produce_one` (Opsi C) — menyerahkan `intelligence.config.TenantConfig`,
  lembar dari baris `channels` yang **tak punya kolom `telegram_*`** (hanya `TenantRunConfig`/`tenant_configs`
  punya). Nomor selalu kosong → keluar `False` **tanpa satu baris log** (pengirim hanya mencatat saat berhasil),
  jadi kegagalannya tak terlihat siapa pun. **Terukur:** 11 video ber-catatan QC (4 channel, 4 tenant,
  19-Jul → 22-Agu) nol pesan; **pembanding di sebelah** 12 video jalur langsung/uji — yang memakai lembar
  `TenantRunConfig` — pesannya terkirim semua. Akibat nyata: video dibuang saat TTL habis, biaya produksi
  hangus, tenant tak pernah tahu ada yang perlu ia putuskan. **Perbaikan = MENYERAGAMKAN, bukan jalur baru:**
  nomor di-resolve lewat `_chat_id_for_tenant` seperti yang SUDAH dilakukan `notify_circuit_break` &
  `notify_published` di berkas yang sama; nama channel diserahkan producer sebagai isian (pola sama).
  Saklar `telegram_enabled` tetap menentukan — diperiksa di lembar **dan** di DB. Penjaga:
  `tests/test_notifikasi_tinjauan_sampai_ke_tenant.py` (8 uji, 2 arah: lembar-tanpa-kolom wajib terkirim ·
  saklar-mati wajib berhenti). **Susulan:** 2 video yang masih tertahan (BJ Yusroon 19-Agu, tenggat <2 jam ·
  Penjaga Dakwah 22-Agu) diberi tahu manual atas izin owner — yang tenggatnya <2 jam memakai jalur teks-bebas
  `notify_tenant` karena kalimat baku "±3 hari" akan BOHONG untuknya.
- 2026-07-10 (3) — **Notif Telegram jalur terjadwal + header circuit-break** (mandat owner; commit `c6f3161`; ⚠️ **KOREKSI 22-Agu: klaim "2 pesan uji nyata terkirim ✓" DITARIK** — yang diuji waktu itu PENGIRIMnya, bukan jalurnya; pada jalur terjadwal pesan (a) tak pernah bisa terkirim sampai 22-Agu, lihat entri 2026-08-22 di bawah): (a) `notify_review_pending` — video masuk antrean Review kini MEMBERI TAHU tenant (judul + catatan QC + saran + arahan Pakai/Buang + peringatan TTL hangus + link /review; chat/toggle per-tenant, TTL & URL config-driven) — menutup celah "video menunggu senyap → TTL → biaya hangus tanpa tenant tahu"; (b) header `notify_circuit_break` seragam `[nama channel]` (dulu UUID mentah).
