...
> ✅ **(a) DIIMPLEMENTASI 2026-06-17 (Lapis 1):** `tts_engine.effective_tts_provider()` resolve provider yang AKAN me-render via **probe ketersediaan** (ElevenLabs: probe sintesis mungil → tangkap `detected_unusual_activity`/kredit-habis yang tak terlihat di endpoint subscription; edge selalu ada). `script_engine` word-budget pakai `delivery_wps` provider EFEKTIF (bukan terkonfigurasi). Tervalidasi: ryan 60s saat ElevenLabs lapse → budget 108→**156 kata** → edge ~60s → QC lolos. *(Lapis 2 closed-loop ukur-koreksi = opsional, belum.)* File: `tts_engine.py` + `providers/tts/{base,elevenlabs}.py` + `script_engine.py`. (preset 8/15/30/45/60/75/90 sudah rapat by-design; turunkan preset = output ikut lebih pendek, tetap meleset — keputusan owner 2026-06-16, no preset-hack).
