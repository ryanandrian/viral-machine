...
> - **TTS DIKEMBALIKAN apa-adanya** (revert "effective-provider/is_available" — itu belok yang salah). File: `script_engine.py` (prompt+gate); `tts_engine.py`/`providers/tts/{base,elevenlabs}.py` (revert bersih). (preset 8/15/30/45/60/75/90 sudah rapat by-design; turunkan preset = output ikut lebih pendek, tetap meleset — keputusan owner 2026-06-16, no preset-hack).
