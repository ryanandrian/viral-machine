...
> ✅ **(a) DIIMPLEMENTASI 2026-06-17 (Lapis 1):** `tts_engine.effective_tts_provider()` resolve provider yang AKAN me-render via **cek ketersediaan**. ElevenLabs `is_available()` = **cek SISA KUOTA** (endpoint subscription: `character_limit−character_count ≥ ambang ELEVENLABS_MIN_CHARS=1000`); edge selalu ada. `script_engine` word-budget pakai `delivery_wps` provider EFEKTIF (bukan terkonfigurasi).
> ⚠️ **Pelajaran e2e 2026-06-17 (verified VPS):** ElevenLabs ryan **bukan mati — sisa 155 kredit** (`quota_exceeded`: 383 char dibutuhkan > 155). Versi awal pakai **probe sintesis mungil** → SALAH (3 char lolos walau kuota tak cukup video nyata) → budget tetap 108 → edge → 50.3s. **Diganti ke cek-sisa-kuota** (unit-validated: sisa 155 → edge → budget 156). *(Lapis 2 closed-loop ukur-koreksi = opsional, belum.)* File: `tts_engine.py` + `providers/tts/{base,elevenlabs}.py` + `script_engine.py`. (preset 8/15/30/45/60/75/90 sudah rapat by-design; turunkan preset = output ikut lebih pendek, tetap meleset — keputusan owner 2026-06-16, no preset-hack).
