...
> - ⬜ **F2 KALIBRASI** (berikut): pace berlapis (voice×niche)→voice→provider dari sampel nyata; hormati `pace_locked`. ⬜ **F3** prompt final + toleransi-1-sumber (cabut hardcode ±10%/±12%/+15% `script_engine` & hidupkan `SCRIPT_LENGTH_TOLERANCE` yang saat ini TIDAK dibaca gerbang — gerbang internal `length_ok` masih hardcode 0.90/1.10). ⬜ **F4** jalur DNA khusus (radiant 1-kalimat; ai_video klip diskrit). ⬜ **F5** swa-kalibrasi + alarm drift (superset [C1] EWMA lama).
