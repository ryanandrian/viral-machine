...
- **Integrasi alur §12c — OPSI A (owner-confirmed 2026-06-16).** Logika publish-private+advisory berada **di DALAM `pipeline.run` blok QC-fail**, jalan **SERAGAM di KEDUA jalur**: producer (`publish=False`) **dan** direct (`publish=True`). **Buffer tetap MURNI** — hanya video QC-pass yang masuk `content_inventory` (producer `mark_ready`); video QC-fail **tidak** masuk buffer. Upload-private QC-fail = **artefak advisory out-of-band**, BUKAN konten terjadwal → invariant §12c (*"publish = slot-driven; producer tak publish konten terjadwal"*) **tetap terjaga**. **Publisher TIDAK berubah** (tetap hanya tangani buffer QC-pass). *(Alternatif "lewat buffer + publisher force-private di slot" ditolak: video gagal tertunda, buffer tercampur, kompleksitas inventory/claim.)*
