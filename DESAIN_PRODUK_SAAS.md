...
**F. QC-fail = Self-Healing (publish PRIVATE + advisory), BUKAN buang (2026-06-16).** Video yang gagal QC (mis. durasi di luar toleransi preset) **tidak dihapus** → di-publish **PRIVATE** (walau channel di-set public) + status `qc_failed` + tenant terima **advisory** (alasan + rekomendasi config, via Telegram) → **tenant putuskan** public/take-down. **Integrasi §12c = Opsi A** (owner-confirmed): logika di `pipeline.run` **seragam untuk producer & direct**; **buffer tetap MURNI** (hanya QC-pass masuk `content_inventory`); upload-private QC-fail = **artefak advisory out-of-band**, bukan konten terjadwal → invariant "publish = slot-driven; producer tak publish konten terjadwal" **terjaga**; publisher tak berubah. Detail desain: `QC_CONTENT_ARCHITECTURE.md §3/§6.2`; checklist status: `PROGRESS.md §IMPROVEMENT`.
