...
- **F3 — Ukur dimensi:** isi `_calculate_viral_score` dari angka nyata (velocity/views YouTube + momentum Trends); LLM hanya meramu angle. **+ presisi relevansi velocity via `videos.list?part=topicDetails`** (kategori topik, 1 unit — robust, bukan keyword-mentah). *Memecahkan M3/M5.*
...
7. **PENDANAAN KUOTA YouTube (owner-confirmed 2026-06-16) = HYBRID:**
   ...
   - **Penghemat kuota NYATA (terverifikasi):** (1) **Batch IDs** — `videos.list?id=v1,v2,…` ≤50 ID = **1 unit total**. (2) **Cache aplikasi `trend_cache`+TTL** — fetch 1×/TTL (lebih andal dari ETag). (3) **"intip channel" (1u) bukan `search` (100u)**. **MITOS (jangan diandalkan):** membatasi `part` **TIDAK** hemat kuota di v3 (biaya `videos.list` flat 1 unit berapa pun part; `part` cuma pengaruh bandwidth) · **ETag/304 TIDAK dijamin** me-refund kuota (request tetap dihitung) → pakai cache sendiri, bukan ETag.
...
- 2026-06-16 (F3a+F3b) — **F3a presisi relevansi:** `topicDetails` ditambah (1 unit sama) → temuan data nyata: topicCategories **terlalu kasar** utk filter positif (relevan sering ke-tag entertainment/kosong) → **mapping-positif dibatalkan**; dipakai sbg **denylist aman** (buang topik MURNI game/musik/olahraga) + LLM penilai utama (`_offniche_topic`). **F3b skor dari angka NYATA:** `_apply_signal_factor` (pola historical_factor) — boost viral_score per-topik yang selaras velocity YouTube (×s/d1.25) / momentum Trends (×s/d1.10); hanya naik, fail-soft, di-wire `select()`. "Ukur, jangan menebak" tanpa refactor berisiko. Keduanya validated. **Sisa = F4 (kalibrasi bobot dari outcome + Analytics kaya) = data-gated post-cutover.**
- 2026-06-16 (F2-velocity LIVE) — **YouTube velocity mining live-validated** (key platform, dev restriction=None / prod=IP VPS): `_get_youtube_trending_search` upgrade → search OR-join → `videos.list statistics` batch → velocity=views/jam (universe_mysteries: 25 video, top 349.504/jam; rantai fetch→cache→scan→summary PASS). Surface views+velocity ke summary LLM. **Temuan:** velocity bawa noise lintas-niche; hard keyword-filter rugikan recall → LLM kontekstualisasi sekarang, presisi robust via `topicDetails` di F3.
- 2026-06-16 (F1+F2 dibangun) — **F1 decouple+cache SELESAI** (migr 0048 `trend_cache` + `TrendRefresher`; produce baca cache, NOL fetch hot-path, anti-429 — validated live). **F2 buildable SELESAI** (migr 0049 `source_weights` §2b; `niche_selector` terapkan bobot via urutan+jumlah-item; **HackerNews niche-filter** [M2]; **Wikipedia di-DROP** dari seleksi [§2b, de-facto unused]; **autocomplete** sumber demand baru — validated unit+live). **GATED (bukan skip):** YouTube velocity (key platform+kuota, owner §7) · F3 ukur-dimensi (butuh sinyal numerik mengalir) · F4 kalibrasi+Analytics-kaya (akumulasi outcome, post-cutover E3). Mekanisme siap; aktivasi = key/data.
