...
- **2026-08-22 — 🧰 PANEL KATALOG AI: prasyarat & koridor DITETAPKAN; 6 butir perbaikan MENUNGGU KETOKAN (nol kode disentuh).** Latar: Batch C (21-Agu) dibatalkan seluruhnya karena membuat PINTU BARU di panel padahal yang diminta memperbaiki jalur yang ADA. Owner menuntut rencana matang & TERCATAT supaya sesi baru (yang "seperti bayi baru lahir") tak mengulang pengerusakan.
  ...
  **BUTIR (urutan wajib B1→B2→B3→B4, lalu B5/B6 hanya sesudah owner menentukan bentuk/tempat):**
  · **B1** lebar jendela pop-up katalog 440px → cukup utk form 15 isian & baris JSON *(satu angka CSS; nol logika)* ⏳
  · **B2** arahan + label manusiawi `ai_models.default_params`, **berubah mengikuti jenis** yang dipilih *(teks di `FIELD_META`; nol validasi baru)* ⏳
  · **B3** isi arahan yang bolong (form voice/moods/durations + `provider_key` di form model) & **sebut langkah lanjutan koridor** §9.1 di arahan yang ADA (tts→langkah 3, video→langkah 4) *(teks)* ⏳
  · **B4** `tts_profiles.display_name`+`adapter` masuk whitelist API ⇒ berhenti dibuang senyap; `ENUM_COLS` yang kini kode mati jadi hidup *(2 nama kolom; nol layar)* ⏳ **perilaku berubah — risiko wajib disebut ke owner lebih dulu**
  · **B5** jejak karantina (`unavailable_since`/`unavailable_reason`) bisa dibersihkan admin — menutup lubang buatan sesi 21-Agu ⏳ **bentuk belum diputuskan owner**
  · **B6** jalur membuat baris `tts_profiles` dari panel ⛔ **satu rancangan SUDAH DITOLAK owner (pintu baru) — JANGAN memilih tempat sendiri lagi**
  **PERTANYAAN yang HARAM DITEBAK** (belum dijawab owner): (a) `request_param_schema` perlu dikelola panel atau sengaja disembunyikan? (b) bentuk B5? (c) tempat B6? (d) fosil `groq` (setelan suara + 2 suara tanpa model tts, migr 0138) — hapus atau biarkan? (e) kolom "dipakai N channel" di tabel AI Models — **tak pernah ada** di 42 versi riwayat; menambahkannya = penambahan layar ⇒ butuh ketokan.
