...
- **⏳ MENUNGGU KETOK OWNER — sengaja TIDAK diputuskan sendiri (§0.6 perilaku-saat-gagal):**
  1. **Jeda sementara** untuk sebab yang pulih sendiri (mesin berhenti mencoba lalu jalan lagi otomatis, **satu** kabar saja) — jalan keluar yang benar untuk KEDUA mudarat. Perlu diketok: **lama jeda** (ikut waktu yang penyedia sebutkan — sudah tersimpan, 56 sampel §8h / jeda tetap / sampai ganti hari) · **jumlah kabar** · **tombol Uji/Jalankan Ulang** menembus jeda atau ikut ditahan.
  2. **Saklar aktif/nonaktif channel tidak menutup periode kegagalan** (migr 0197 mewajibkannya untuk *setiap* jalur pelepas rem; jalur saklar terlewat). BISIK & Thetangga menyimpan hitungan **12** ⇒ begitu tenant menyalakannya, mesin mengerem **seketika tanpa satu percobaan pun**. Tindakan datanya menyentuh data tenant ⇒ butuh izin.
  3. **Jam tenant menyalakan/mematikan channel tidak terekam** — diagnosa insiden bergantung pada keterangan owner/tenant.
  4. **Cloudflare `5006` + parameter `seed`** — salah KITA ditimpakan ke tenant (*"Kegagalan terjadi di layanan AI Anda"* untuk permintaan kita yang cacat); 32 sampel nyata, tren naik. Akarnya: **`seed` tidak ada di skema resmi FLUX schnell** (hanya `prompt`+`steps`, dibaca 14-Agu) — mencabutnya membuang fungsi *Diversity* §9.1, jadi bukan satu baris.
- **DONE-BILA:** (1) banjir tertutup — mesin berhenti sendiri di ambang ✅ · (2) nol channel aktif jadi korban pencabutan ✅ (diukur) · (3) dokumen SSOT berhenti menyatakan perilaku yang tak ada ✅ · (4) penjaga berbasis PERILAKU + 3 penjaga dokumen, merah dibuktikan ✅ · (5) ⏳ **deploy** (gerbang izin terpisah) · (6) ⏳ 4 butir ketok owner di atas.
