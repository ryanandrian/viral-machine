...
---
### 🔴🔴 PEKERJAAN MENGGANTUNG — BACA SEBELUM APA PUN (27-Agu-2026)

**1. Kode program AGEN sudah SELESAI & TERBUKTI tapi BELUM TERSIMPAN di riwayat.** Ada di direktori
kerja (`git status`), 12 berkas. **Gerbang commit MENOLAK** karena satu uji **tak terkait** merah:
`test_patri_larangan::test_NOL_prompt_produksi_lama_diblokir` — mesin naskah menyelipkan satu kata
Arab (**سلطان**) ke prompt gambar adegan **vila** (25-Agu), lalu penyaring patri memblokirnya (benar:
huruf Arab yang ditulis mesin gambar sering rusak). **Owner: "biarkan saja dahulu, jangan menjudge
sebelum lihat hasilnya."** ⇒ jangan sentuh patri; kode agen menggantung sampai owner memutuskan.
**Yang SUDAH berlaku di DB (tak menunggu apa pun):** atribusi `andarini.nadia`→AGEN01 + komisi
**Rp 8.725** lahir + agen dikabari Telegram · migr **0217** (pagar atribusi permanen) terpasang.
Rinciannya: `AGENT_AND_AFILIATION_ARCITECTURE.md` **§9e**.

**2. Rencana JATAH TOKEN NASKAH siap, menunggu ketok owner** → `QC_CONTENT_ARCHITECTURE.md` §2c
(blok "🔴 JATAH TOKEN NASKAH KEKECILAN"). Inti: `max_tokens=2000` ditanam di kode, sementara **51%
naskah melebihinya** (diukur `tiktoken` pada 182 naskah) ⇒ vendor ketat (Groq) gagal total, vendor
longgar bayar 2×. Usul: 2.000→6.000 & batas atas 4.000→8.000; **nol tambahan biaya**; prompt/gerbang
durasi/refit per-segmen TIDAK disentuh.

---

### Changelog
- **2026-08-27 — 🤝 ATRIBUSI AGEN HILANG DI PINTU GOOGLE — DITUTUP + KOMPLEN AGEN01 DIJAWAB (ketokan owner).** **Pemicu:** agen AGEN01 (THETANGGA) komplen `andarini.nadia` seharusnya tercatat sebagai pelanggan bawaannya. Diperiksa ke DB: tabel atribusi **0 baris** · ketiga kode rujukan **dipakai 0 kali** · buku komisi **0 baris** — padahal program agen hidup sejak 19-Jul. **AKARNYA 100% karya saya:** SSOT §5a merancang atribusi lahir dari *"form daftar"* tanpa membedakan pintu; halaman daftar punya **DUA** pintu (email & Google), saya lengkapi pintu email dan **tak pernah menanyakan pintu Google** yang sudah ada lebih dulu. Terukur: **14 dari 14** tenant sejak 19-Jul memakai Google ⇒ satu-satunya jalur atribusi yang berfungsi **tak dipakai siapa pun**, padahal layar sudah berjanji *"✓ Kode valid — pendaftaran Anda tercatat lewat mitra kami"*. Ini **mata ke-5 lima-rantai (jalur saudara) yang dilewati**. Dan tak ketahuan lebih awal karena kegagalan atribusi hanya masuk jejak audit yang **nol pembaca** — sementara "audit A–Z" §9b yang saya tulis mengaku memeriksa *"integrasi (signup…)"* padahal memeriksa pintu email saja, lalu menyatakan *"JANGAN audit ulang fase teknis"*. **YANG DIKERJAKAN, semuanya di jalur yang SUDAH ada (nol pintu/tabel/rute baru):** (1) pintu Google **menitipkan** kode rujukan (cookie 10 menit; sengaja BUKAN parameter pada alamat pulang OAuth — itu divalidasi daftar-izin Supabase, dan memperbaiki atribusi tak boleh berisiko mematikan seluruh login Google) → penerima menulis atribusi · (2) **pagar §1b**: hanya akun **baru lahir** (≤120 detik, angka dari data nyata) — tenant LAMA tak bisa diklaim agen lewat tautan rujukan · (3) **pagar DATABASE** (migr `0217`): atribusi tak bisa di-UPDATE/DELETE oleh penulis mana pun, sebab sejak perbaikan ini penulisnya DUA · (4) kegagalan atribusi **dialarmkan ke admin** di KEDUA pintu (memakai jalur internal + `notify_admin` yang sudah ada) — supaya kelas ini tak bisa senyap lagi · (5) kenop **"Komisi Default (Agen Baru)"** (SSOT §4) yang ternyata **nol pembaca** disambungkan (API tak mengambilnya, dan hanya membaca kolom `value` padahal kenop teks tersimpan di `value_text`) · (6) **pemulihan sekali jalan**: `andarini.nadia` ditautkan ke AGEN01 + komisi pembayaran 26-Agu dilahirkan lewat otoritas yang ada = **Rp 8.725** (25% × Rp 34.900), **agen otomatis dikabari Telegram**. **BUKTI:** 10 penjaga (**9 dibuktikan MERAH dulu**) · **10 sabotase semuanya tertangkap** — dan sabotase-lah yang menemukan **4 penjaga palsu buatan saya sendiri**, termasuk satu yang **tak menangkap cacat aslinya** (pola substring; jendela menemukan DEFINISI fungsi alih-alih PEMANGGILANNYA) · build FE lulus · migr `0217` dijalankan KERING dulu. **Pengecualian §1b ("tidak ada klaim belakangan") tercatat BERTANGGAL di SSOT** — bukan pelunakan aturan: ia hanya berlaku bila atribusi hilang karena kegagalan sisi KAMI. **Batas jujur:** penjaga pintu berbasis TEKS (menangkap pencabutan, bukan pelumpuhan isi — proyek belum punya penjalan uji layar), karena itu pagar terpenting dipindah ke DB & diuji BERTRANSAKSI; dan tak bisa dibuktikan dari sistem bahwa nadia datang lewat tautan AGEN01 (jejaknya tak pernah ada — itu inti cacatnya). SSOT: `AGENT_AND_AFILIATION_ARCITECTURE.md` **§9e** (+ §1b · §4 · §5a diperbarui). ⏳ **MENUNGGU IZIN DEPLOY.**
- **2026-08-24 — 🔀 BIAYA TAK BOLEH TERTUKAR ANTAR PENYEDIA · GEMBOK HARGA TAK LAGI DILUPAKAN · SINKRON BERHENTI PERCAYA "200 OK" (A·B·C, ketokan owner).** Tiga perbaikan **bug di jalur yang sudah ada** — nol jalur baru, nol kolom baru, nol layar tersentuh. **(A)** Owner: *"yang harus dipastikan tidak overlaping adalah model yang sama tapi dari provider yang berbeda, masing-masing punya harga yang berbeda."* Terbukti bug: meteran biaya hanya mencatat **nama model**, dan penanda model **tak punya aturan keunikan** ⇒ dua baris bernama sama, yang kedua **menimpa** yang pertama tanpa suara. Hari ini aman **karena kebetulan** namanya berbeda — tapi router (APIMaster/OpenRouter) menyebut model dengan nama **persis sama**, dan selisih harganya **150×**. Kini penyedia ikut jadi bagian kunci (titik pencatat sudah tahu penyedianya), bentuk kuncinya hidup di **satu tempat**, dan nama tanpa penyedia yang **ambigu** dilaporkan **jujur belum-terhitung** alih-alih ditebak. Ambang: **nol produksi yang baru jadi belum-terhitung**; seluruh selisih angka terjelaskan tepat. **(B)** Alarm harga-basi ternyata **sengaja melewati baris terkunci** — persis terbalik: yang otomatis dijaga (padahal memutakhirkan diri sendiri), yang ketikan tangan (satu-satunya yang BISA basi) tak dijaga apa pun. Kini dua kelompok dua jendela (otomatis 7 hari · ketikan tangan **30 hari**, angka mati sesuai ketokan owner), **tanpa tanggal = belum pernah dipastikan**, pesannya **memisahkan dua sebab** dan menyebut cara mematikannya lewat jalur yang sudah ada (✎ → Simpan) — nol tombol baru. Migr `0216` memberi tanggal pemeriksaan sebenarnya pada 6 baris; **dibuktikan pada 47 baris nyata: nol alarm palsu**. **(C)** Diuji ke API nyata: **API harga fal menjawab "200 OK" untuk endpoint yang TIDAK ADA** — jadi satu salah ketik penanda model bisa menulis harga untuk model yang tak pernah ada; kita selamat hanya karena satuannya tak dikenali (kebetulan, bukan pemeriksaan). Kini **harga tidak ditulis untuk model yang belum pernah lulus tombol Uji** (42 baris aktif semuanya lulus ⇒ nol kehilangan), dan satuan vendor yang belum dikenal **dialarmkan**, bukan cuma masuk log. **Bukti:** 9 uji dibuktikan MERAH dulu · **10 sabotase semuanya tertangkap** — satu di antaranya menemukan penjaga palsu buatan saya sendiri (melumpuhkan penyusun kunci di meteran lolos semua uji lain) · 131 uji rantai biaya hijau · migrasi dijalankan KERING dulu. **Utang yang saya betulkan sendiri:** butir C sempat membuat 3 uji lama merah — prasyaratnya **dipenuhi di data uji**, bukan dilonggarkan di kodenya. SSOT: `ARSITEKTUR_AI_PROVIDER_MODEL.md` §7f. ✅ **TERPASANG DI PRODUKSI 25-Agu 02:46 WIB (izin owner, commit `fcc9ed5`)** — mv-worker/mv-webhook aktif, health 200, worker restart bersih; FE tak ikut (nol berkasnya berubah). **Dibuktikan SEBELUM deploy, bukan sesudah:** `scripts/gladi_biaya_channel_aktif.py` menjalankan rantai biaya yang sama dgn produksi memakai katalog nyata + setelan 9 channel tenant sesungguhnya (jawaban vendor ditiru; nol panggilan vendor, nol kredit) → **9 dari 9 channel terhitung penuh**.
- **2026-08-23 (2) — 🧭 RENCANA F1–F8 rantai harga & biaya DITETAPKAN, progresnya dilacak di SSOT.** Sesudah riset sehari penuh (10 cacat, semuanya karya saya sendiri; 4 kebenaran ditetapkan; APIMaster & OpenRouter diuji dengan perintah produksi nyata). ✅ **TERPASANG DI PRODUKSI 23-Agu 20:15/20:18 WIB (izin owner, commit `516fad1`)** — mv-worker/mv-webhook/mv-web aktif, health 200, worker restart bersih. Migrasi 0209-0215 sudah diterapkan. **PENUNJUK TUNGGAL: `ARSITEKTUR_AI_PROVIDER_MODEL.md` §7f-0 (MULAI DARI SINI: keadaan deploy, migrasi yang sudah diterapkan, tindakan owner yang menunggu, langkah berikutnya) lalu §7f** — di situ rencana, status tiap langkah, **15 formula**, aturan mengikat, dan angka acuan. Jangan menyentuh rantai harga/biaya tanpa membaca §7f lebih dulu. ✅ **F1–F4 SELESAI 23-Agu** (migr `0210` 47/47 ber-formula · penghitung memakai formula, 246 run → **246 identik** · migr `0211` URL sumber harga jadi kenop admin + **pagar agregator**) · migr `0212`/`0213` API harga resmi penyedia → **7 dari 12 model fal otomatis** (3 naskah fal kini **$0,001/permintaan** = tarif fal yang benar) · ✅ **F7 SELESAI 23-Agu** (migr `0214`): dua tarif yang **mesin mustahil menangkap** dibetulkan dari halaman resmi vendor lalu **DIKUNCI + bercatatan sumber & tanggal** — suara Gemini $0,3/$2,5 → **$0,50/$10,00** (4 channel aktif; komponen suara **×3,96**, total per video **+4,1…+4,5%**) · `eleven_v3` $180 → **$100** (umpan memberi tarif kelebihan-kuota; 1,8× lolos penjaga lonjakan yang butuh 3×) · 5 baris lain angkanya sudah benar tapi **tanpa jejak asal** diberi catatan tanpa menyentuh nilainya, termasuk Cloudflare beserta **aritmetika kuota gratisnya** (96 neuron/gambar ⇒ 104 gambar/hari gratis; puncak nyata 22/hari). Penjaga **G13** menegakkan §7c yang sejak 22-Agu **nol mesin memeriksanya** — dibuktikan MERAH dulu (5 dari 6 baris melanggar) + **5 sabotase tertangkap** · ambang: migrasi KERING dulu (7 baris bergerak, tepat 2 berubah angkanya, nol formula bergeser) · 246 run dihitung ulang → **244 identik, 2 terjelaskan, nol tak terjelaskan** · **1391 uji hijau** · riwayat tenant TIDAK ditulis ulang (layar membaca angka tersimpan per produksi) · **berlaku tanpa deploy**. · ✅ **F5 SELESAI 23-Agu** — kalau vendor MENYEBUT biayanya sendiri, mesin memakai angka itu dan **berhenti menaksir**: seluruh rantai "jumlah yang kita ukur × tarif yang kita simpan" (tempat 10 cacat kemarin hidup) tak dilewati sama sekali. Diverifikasi ke dokumen resmi OpenRouter: `usage.cost` selalu dikirim, 1 credit = 1 USD, dan dibaca dari balasan yang **sudah** kita terima ⇒ **nol panggilan tambahan, nol kredit**. Keranjang meter **sendiri** ⇒ mustahil tertagih dua kali; baris begini tak butuh tarif, dan panel **berhenti memberi peringatan palsu** "harga kosong" padanya (daftarnya dari cermin DB, nol nama formula diketik di layar). Nama kolom vendor **tidak ditebak** — vendor yang memakai nama lain muncul di laporan harian, bukan salah diam-diam. Bukti: `G14` 8 uji (4 merah dulu) + **11 sabotase tertangkap** · 0 run & 0 baris model memakai jalur ini ⇒ nol geseran riwayat, dan ke-19 run yang beda dari angka tersimpan **semuanya bernama, nol tak terjelaskan** · **1399 uji hijau** · build FE lulus. ⛔ **Separuh F5 sengaja TIDAK dipasang** ("selisih penghitung akun"): produksi berjalan **serentak**, jadi selisih penghitung satu akun mustahil dibebankan ke satu produksi — dua produksi tenant yang sama akan saling mencuri biaya. Tempatnya di rekonsiliasi tingkat tenant (**F8**); **menunggu ketokan owner**. · ✅ **F6 SELESAI 23-Agu** — lencana **"✓ Teruji"** berhenti bisa berbohong. Dulu tombol Uji naskah cuma minta *"jawab satu kata: OK"* dengan jatah 512 token; produksi minta JSON dengan jatah 1.200–2.000. Terukur: **4 dari 6 model APIMaster lulus uji pendek itu lalu GAGAL** pada perintah naskah asli — dan gerbang DB yang menolak menyalakan model tanpa stempel LULUS **ikut tertipu**. Kini ujinya memakai kontrak yang SAMA dengan produksi (minta JSON · jatah penuh, kenop admin · diurai parser yang sama · kunci yang produksi baca wajib ada), dan vonis gagalnya memisahkan *"balasan kosong"* dari *"balasan ada tapi tak bisa dipakai"* supaya admin tahu tindakannya. Bukti: 7 uji (5 merah dulu) · **5 sabotase tertangkap**, dan sabotase-lah yang menemukan celah di penjaga saya sendiri (JSON sah tanpa kunci produksi tetap lolos) · **1406 uji hijau** · build FE lulus. **Batas jujur:** 20 dari 20 model naskah aktif masih ber-stempel LULUS dari uji LAMA yang lemah — stempelnya TIDAK dibatalkan (itu keputusan owner); menekan Uji sekali memperbaruinya. Uji **suara** masih pakai frasa pendek (belum ada bukti kegagalan; uji sepanjang produksi berbiaya tiap tekan) — menunggu ketokan. · 🟡 **F8 SEBAGIAN SELESAI 23-Agu — dan SSOT rantai harga/biaya DITUTUP.** Alarm yang sudah ada hanya menyala kalau mesin **mengaku** gagal; yang berbahaya justru sebaliknya — angka keluar, tampak wajar, nol alarm (itu bentuk insiden 22-Agu: 16 produksi, biaya suara Rp 0, semua diam). Kini ada pemeriksaan harian atas dua tanda yang **berarti uang nyata tak tertagih**: panggilan tercatat tapi pemakaiannya nol · biaya total Rp 0 padahal ada pemakaian dan mesin tak mengaku gagal. Nol panggilan ke vendor, gagal-lunak, 1×/hari. Bukti: 6 uji merah dulu · **6 sabotase tertangkap** (termasuk "berhenti dipanggil petugas harian" = kode mati) · dijalankan pada data produksi nyata → **nol temuan, nol alarm palsu** · **1412 uji hijau**. ⛔ **Yang TIDAK bisa hari ini:** membandingkan ke pemakaian nyata akun vendor — 9 penyedia aktif tak menerbitkannya, membaca tagihan akun tenant = keputusan owner (BYOK, itu akun mereka), dan satu kunci tenant bisa dipakai di luar MesinViral sehingga selisihnya memuat belanja yang bukan milik kita. Jalannya sudah disiapkan F5 (penyedia router melaporkan biayanya sendiri). **Kematangan: naik dari *crawl* ke *walk*.** 🔎 **Dua temuan dilaporkan, TIDAK diubah sendiri** (menyentuh layar ⇒ ketokan owner): rincian biaya per komponen dihitung & disimpan di SETIAP produksi tapi **nol pembaca** — dan kelas CSS untuk menampilkannya **sudah ada** tapi tak dipakai (layarnya pernah dirancang lalu tak disambungkan); serta model gambar ber-tagih token masuk baris "naskah" bukan "gambar" (82 produksi — total benar, rincian salah alamat). · ✅ **F4b SELESAI 23-Agu** — empat baris fal terakhir. fal menagih gambar per **megapiksel dibulatkan KE ATAS** dan video seedance per **token** `(tinggi×lebar×fps×durasi)÷1024`; katalog kita menyimpannya sebagai angka yang dihitung TANGAN untuk satu ukuran saja — benar hari ini, berhenti benar **tanpa suara** begitu resolusi/fps/durasi berubah. Kini faktanya **DIUKUR dari berkas hasil** (ukuran gambar dari berkasnya; dimensi & **fps** klip dari ffprobe — kita tak pernah menyebutkan fps ke vendor, jadi angka katalog itu tebakan), pembulatan terjadi **per gambar** seperti cara vendor menagih, dan gagal-lunak: satu fakta tak terukur → dilaporkan jujur belum-terhitung, produksi jalan terus. **Nol migrasi, nol tulisan tangan** — sinkron harian menulis tarif DAN formulanya sendiri sesudah deploy; dibuktikan sinkron KERING ke API resmi fal (nol kredit). **Nol geseran angka hari ini**: hasil formula baru pada ukuran yang dipakai sekarang **persis sama** dengan angka manual yang tersimpan (aritmetikanya ada di SSOT §7f) — yang dibeli adalah angka yang tetap benar bila ukurannya berubah. veo tetap **dikunci** (API fal menyebut tarif beraudio, pipeline kita tanpa audio). Bukti: 13 uji (14 merah dulu) · **11 sabotase tertangkap, 3 di antaranya menemukan celah di penjaga saya sendiri** — dua uji memakai `assertFalse` pada nilai yang kebetulan 0 sehingga lolos saat penolaknya dicabut, dan **mengganti pengukuran berkas dengan angka tetap 1080/1920/24 lolos seluruh uji lama** padahal itu inti F4b · 246 run dihitung ulang: nol geseran baru · **nol channel aktif** memakai flux/seedance ⇒ radius nol bagi tenant · **1425 uji hijau** · build FE lulus. ⚠️ **URUTAN WAJIB: deploy dulu**, baru sinkron memutakhirkan keempat baris itu — jangan dibalik. · ✅ **Alamat rincian biaya dibetulkan (23-Agu)** — baris rincian kini ditentukan JENIS MODEL, bukan keranjang meter. Terukur: total **tidak bergeser sama sekali**, tapi **101 dari 246 produksi** rinciannya salah alamat — biaya gambar tersembunyi di baris naskah sementara baris gambar terbaca Rp 0. Riwayat tersimpan tidak ditulis ulang. Ini bukan keputusan owner: nol layar tersentuh, nol total berubah, dan alamat yang salah tak punya pembelaan. · ⏳ **Dua butir menunggu KEPUTUSAN owner** (bukan pekerjaan terlupa): **menampilkan** rincian biaya per komponen di layar tenant (angkanya sudah benar & tersimpan, tapi nol layar membacanya — padahal kelas tampilannya sudah ada tanpa dipakai) · uji suara sepanjang produksi. Plus satu **saran saya**: `selisih_akun` jangan dipasang per-produksi. · ✅ **SEMBILAN tarif yang sungguh dipakai diperiksa ke halaman resmi vendornya (23-Agu)** — OpenAI, Google, Groq. **Delapan tepat, satu salah**: `gpt-image-1-mini` tarif MASUK-nya terlalu murah (keluarnya sudah benar) → dibetulkan + dikunci (migr `0215`, sudah diterapkan); terukur 82 dari 246 produksi, biaya naik **+0,5%…+1,5%**. Delapan lainnya sengaja dibiarkan otomatis supaya tetap mengikuti perubahan vendor. **Artinya seluruh tarif yang menyentuh tenant aktif kini terbukti**, bukan diwarisi dari sumber pihak ketiga. Sekalian terjawab: usulan harga tertahan `gemini-2.5-flash-image` **bukan lonjakan liar** — halaman resmi Google menyebut angka itu sebagai penyebutan lain dari harga per-gambar yang sudah kita pakai, jadi **boleh diabaikan**.
- **2026-08-23 — 🧮 RANTAI HARGA & BIAYA DIBERESKAN DI AKARNYA: satu daftar satuan, tiga pembaca, 15 gerbang (S-DOC + S0–S3, S5; izin owner).** Pemicu: owner minta periksa satu angka biaya; tiap pemeriksaan ulang memunculkan cacat baru sampai **10 temuan**, semuanya karya saya, semuanya SENYAP. **AKARNYA SATU:** pengetahuan *"satuan apa berlaku untuk jenis apa, dihitung sekali di mana"* tersebar di **4 tempat** yang tak saling tahu ⇒ tiap tempat membusuk sendiri. Kosakata FinOps **FOCUS** (diperiksa; **tidak** diadopsi sebagai format — ia untuk ekspor tagihan penyedia, kita BYOK) menamai celahnya: harga tersimpan dalam **satuan TAGIH**, pemakaian tercatat dalam **satuan TERUKUR**, dan kita mengalikan keduanya tanpa konversi. **YANG DIKERJAKAN:** **S-DOC** `ARSITEKTUR_AI_PROVIDER_MODEL.md` **ditetapkan SSOT** (§7a definisi metrik + apa yang MASUK/TIDAK + asumsi · §7b tabel satuan kanonik · §7c asal tiap angka · §7d catatan pembandingan luar bertanggal · §7e batas jujur + kematangan *crawl→walk* + putusan FOCUS); `DESAIN §10` tabel tarif Juni **DICABUT** (salah 3×: satuan gambar, tarif ElevenLabs 4×, menyebut Pexels/R2 yang pensiun); `AI_VIDEO_8S_PLAN` tarifnya ditandai **CUPLIKAN—BUKAN ACUAN**; `finalisasi_tier_plan`+`PAYMENT` diberi pembeda *harga langganan ≠ harga model AI*; **`MEMORY.md` menunjuk SSOT** (sebelumnya tidak → sesi baru buta terhadap dokumen buatan saya sendiri). **S1** `SATUAN_HARGA` = sumber tunggal; penghitung biaya **satu putaran atas daftar**, nol cabang per-kasus; **prioritas skema ⇒ satu model satu tagihan** (hitung-ganda mustahil secara struktur, bukan dijaga `if`); harga **esensial** per skema (separuh harga = angka masuk-akal-tapi-salah ⇒ skema tak berlaku). **S2** sinkron sadar-jenis: kolom umpan yang **bermakna ganda ditolak** (`output_cost_per_token` haram utk suara/gambar); prefix umpan **hanya dari DB** (daftar 7 prefix tertanam dibuang — 0 model bergantung padanya; mencocokkan ke prefix vendor lain = harga vendor lain). **S3** huruf suara dicatat **sekali** (pencatat ganda di adapter Gemini dicabut: vendor terima 1113, meter catat 2226); **detik audio** dicatat ⇒ `gpt-4o-mini-tts` **terhitung otomatis** tanpa menyentuh jalur audio produksi. **S5** satuan+label **dicerminkan ke DB** lewat `catalog_sync` yang sudah ada (`pricing_unit:<jenis>`, migr **0209** izin baca) → panel & layar tenant membacanya: **nol nama satuan diketik di kode layar**, dan cabang video yang hilang di layar tenant (model video tampil "/gambar" berbulan-bulan) sembuh sendiri; kurs hanya dari `app_config` (angka 16500 di 2 layar dibuang; kurs belum ada → tampil USD, bukan Rp palsu). **BUKTI:** **15 gerbang** dibuktikan **MERAH dulu** lalu hijau, **11 sabotase** semuanya tertangkap (2 di antaranya menangkap penjaga saya sendiri yang masih bisa jadi kode mati) · **244 run riwayat dihitung ulang: 227 identik, 17 berbeda dengan SATU sebab sama** (hitung-ganda gambar Gemini, −7,2%…−7,7%), **nol selisih tak terjelaskan** · sinkron **kering** 47 baris → **3 berubah**, 44 tak bergerak · **1363 uji hijau** · tsc + build FE lulus · nol data tenant disentuh, riwayat tak ditulis ulang. **SISA:** dasar tagih agregator **fal** belum diverifikasi ke dokumen vendor (S4; 0 channel aktif) · pemeriksaan harian catatan-vs-kenyataan. ⏳ **MENUNGGU IZIN DEPLOY.**
- **2026-08-22 (3) — 💰 BIAYA SUARA DILAPORKAN Rp 0 DI 4 CHANNEL AKTIF; MESIN BIAYA BERHENTI BISA GAGAL SENYAP (A–E, izin owner).** **Akar:** Google menagih `gemini-2.5-flash-preview-tts` **per token**; umpan harga mencatatnya apa adanya (`in_per_1m`/`out_per_1m`), sedangkan penghitung biaya kita hanya mengenal SATU satuan untuk suara (per sejuta huruf) ⇒ **16 produksi, biaya suara Rp 0**. Bukan angka salah — **satuan** yang tak terbaca. Kelas yang SAMA sudah pernah ditemukan & ditutup untuk GAMBAR (`gpt-image-1` per-token → dihitung di bucket llm) dan pertanyaan yang sama **tidak** saya tanyakan untuk suara — mata ke-5 (jalur saudara) dilewati dua kali di topik yang sama. **A** mesin suara Gemini membaca hitungan token dari balasan vendor (`usageMetadata`, pola yang sudah terpasang di mesin gambar) → keranjang meter baru `tts_tokens` → `ai_cost` menghitungnya; per-huruf tetap DULU ⇒ mustahil ganda; vendor tak mengirim hitungan ⇒ **jujur** masuk `unpriced` (haram menaksir token dari jumlah huruf). **B** `report_unpriced_models` — laporan harian **berbasis BUKTI PRODUKSI** (`cost.unpriced`, jendela 3 hari, alarm admin 1×/hari) di petugas harian; **sengaja bukan** daftar aturan per-jenis: vendor baru bisa memakai satuan yang belum ada hari ini, daftar begitu PASTI tertinggal. Inilah yang membuat penambahan provider/model baru ke depan tak bisa lolos senyap untuk **keempat** jenis. **C** panel: editor harga menuntun **per jenis** (satuan mana yang berlaku) + kolom harga menyebut **asalnya** (`otomatis`/`manual`) — sebab **16 dari 42 model aktif TIDAK ADA di umpan publik** (5 model video, ElevenLabs, fal, Cloudflare, Edge) sehingga harganya wajib diketik. **D** penjaga untuk keempat jenis × tiap satuan. **E** layar tenant berhenti menjanjikan lebih: kolom & kartu Biaya AI kini **"perkiraan dari tarif resmi"** (bukan "nyata"), penanda `≈` netral menggantikan ⚠️, **kode internal model tak lagi dicetak ke tenant**, dan kartu dashboard berhenti menjumlahkan angka kurang tanpa penanda (dulu: paling menyesatkan — diam-diam kurang). **Mesin sinkron harga TAK disentuh** (perilakunya benar). **Bukti:** 33 uji baru dibuktikan MERAH dulu · 19 sabotase, **4 di antaranya menangkap uji palsu buatan sendiri** (jendela jatuh ke definisi fungsi · jendela terputus sebelum kodenya · penjaga tetap hijau saat isinya diganti `{"·"}` · penjaga tetap hijau saat syaratnya diganti `false ?`) · **1348 uji hijau** · build FE lulus · run 503 & 504 dihitung ulang dengan mesin baru = **byte-identik** dengan yang tersimpan. **Batas jujur:** `gpt-4o-mini-tts` tetap tak bisa dihitung (vendor mengirim audio mentah tanpa hitungan token; **nol channel** memakainya) — berlabel jelas + masuk laporan harian. Riwayat tenant **tidak** ditulis ulang. Nol migrasi, nol kolom baru, nol rem/kuota tersentuh. ⏳ **MENUNGGU IZIN DEPLOY.**
- **2026-08-22 (2) — 🔔 PESAN "VIDEO MENUNGGU KEPUTUSAN" TAK PERNAH SAMPAI + RUN GAGAL BERBOHONG SESUDAH DIULANG (P1–P3, izin owner).** Dua laporan owner, keduanya **BENAR**, keduanya terverifikasi ke kode & DB. **(P1)** Pesan tinjauan pada jalur TERJADWAL mustahil terkirim — producer menyerahkan lembar setelan tanpa kolom `telegram_*`, pengirim keluar `False` **tanpa satu baris log**: **11 video ber-catatan QC (4 channel, 4 tenant, 19-Jul→22-Agu) nol pesan**, sementara 12 video jalur langsung/uji terkirim semua. Diperbaiki dengan MENYERAGAMKAN ke pola `notify_circuit_break`/`notify_published` yang sudah ada di berkas yang sama (resolve nomor dari `tenant_configs`), + nama channel diserahkan producer. Saklar tenant tetap menentukan (diperiksa di lembar DAN di DB). **Susulan atas izin owner: 2 video tertahan diberi tahu manual** (BJ Yusroon 19-Agu tenggat <2 jam → jalur teks-bebas `notify_tenant` karena kalimat baku "±3 hari" akan bohong · Penjaga Dakwah 22-Agu → jalur produksi yang baru diperbaiki; **keduanya terkirim ✓**). **(P2)** `direct_jobs.source_run_id` = **kolom mati** (nol pembaca) ⇒ sesudah ulangan sukses, baris asal tetap "Gagal" tanpa keterangan dan tombol ulang tetap hidup (sekali tekan = kredit AI tenant terbakar; terjadi 22-Agu, 1 produksi 6m37s). Kini kedua layar menyebut hasil + **nomor run hasilnya** ("sudah diulang dan sukses · RUN #504", tertaut) — nomor dari baris `production_runs`, bukan dirakit dari teks; tombol MATI saat ulangan berjalan/berhasil, tetap HIDUP bila ulangan juga gagal (jalur buka wajib ada). Baris riwayat **tidak** dihapus. **(P3)** Kalimat panel "Produksi langsung" berhenti menjanjikan progress yang layar itu tak tampilkan — halaman run **sudah** punya penanda 8 langkah + log berjalan, tapi baru bisa dibuka SESUDAH selesai (baris run ditulis di akhir; menggeser waktu penulisannya = menyentuh buku-besar yang dibaca rem darurat ⇒ **tidak** dilakukan). **Bukti:** 18 uji baru dibuktikan MERAH dulu · 13 sabotase (2 di antaranya menangkap uji palsu buatan sendiri: jendela teks jatuh ke komentar, dan penjaga yang lolos saat aturannya diganti `= false`) · 1315 uji hijau · build FE lulus · nilai render run 497 diuji ke data nyata. **Nol kolom/tabel baru, nol migrasi, nol data tenant disentuh.** SSOT: `QC_CONTENT_ARCHITECTURE` changelog 22-Agu (+ koreksi klaim palsu 10-Jul) · `DESAIN_PRODUK_SAAS §A` (klaim "progress live di /runs/[id]" dikoreksi) · `PAYMENT §10a` (rujukan baris basi → nama fungsi). ⏳ **MENUNGGU IZIN DEPLOY.**
