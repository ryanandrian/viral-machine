...
---
### Changelog
- **2026-08-20 (4) — 🔐 KUNCIAN KLAIM CHANNEL YOUTUBE: rencana + arsitektur + progress DITULIS, kerja BELUM dimulai.** Ketokan owner: tenant masa coba boleh daftar ulang, tapi **tidak boleh membawa channel yang sudah terdaftar di akun lain** (*"kuncinya bukan di pendaftaran, tapi di integrasi"*). **SSOT rencana & progress = `CHANNEL_LOCK_ACTIVATION_PLAN.md` §7** (sesi baru: baca §7 lalu lanjut dari baris ⬜ pertama di §7f). Terukur: dua indeks unik yang ada di-scope **per-tenant** (migr 0146) · `disconnect()` **menghapus** baris pool sehingga kuncian di tabel itu akan lenyap · jangkar `trial_started_at` mengikat **akun**, bukan channel · **0** channel dipakai >1 tenant hari ini (lubang masih bersih) · **4 tenant** punya >1 channel di satu akun Google (⇒ kunci wajib pada identitas channel) · trend-radar pakai kunci API, **bukan** OAuth (tidak ikut terkunci). **Evaluasi final menangkap 4 hal yang akan jadi bug baru:** (1) jangan cabut token ke Google saat menolak — berisiko membatalkan grant lain dari akun Google yang sama; (2) celah antara migrasi dan deploy → isi-mundur **wajib diulang** pasca-deploy; (3) saklar induk `app_config.klaim_channel_aktif` belum ada; (4) klaim wajib diperiksa **sebelum** dedup se-tenant. Matriks *"setiap kunci punya jalur buka"* (`PAYMENT §10e-2`) sudah ditambah barisnya. **Batas jujur:** kuncian ini menghentikan produksi gratis berkelanjutan ke channel nyata, **bukan** pendaftar ulang yang tak pernah menyambung YouTube (jatah uji akun, kini 3) — jangkar lain = keputusan produk terpisah. Nol perubahan kode di sesi ini.
