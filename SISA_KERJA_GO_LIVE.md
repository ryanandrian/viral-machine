...
---
### Changelog
- **2026-08-21 — 🔌 PINTU KEDUA TERSAMBUNG: channel yang DIAM akhirnya bersuara (Batch A, migr `0204`).** Owner melaporkan 2 kegagalan; penelusuran menemukan yang **tidak** dilaporkan: **4 channel mati 4 hari** sejak 17-Agu (2 tenant **BERBAYAR** langganan aktif) karena model naskahnya dimatikan di katalog **tanpa seorang pun diberi tahu** + log dibanjiri 20.979 baris/5 hari. **SSOT rencana & rincian = `AI_ERROR_MANAGEMENT_ARCHITECTURE.md` §9b** (+ rencana lengkap 4 batch di berkas rencana sesi). Sesi baru: baca §9b, lanjut **Batch B** (langkah 4 · 6a karantina bebas-biaya · 6b konfirmasi berdampak). Prinsip owner yang mengikat: **jangan sentuh database tenant** — sistem yang bersuara, tenant yang memilih penggantinya. Bukti Batch A: 1211 uji hijau · 12 uji dibuktikan MERAH dulu · 11 sabotase merah · rujukan menggantung **tetap 5 (4 aktif)** ⇒ nol data tenant tersentuh · 16 label `channel_missing` utuh. ⏳ Belum deploy.
